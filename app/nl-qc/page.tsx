...
export default function NLQCPage() {
  ...
  const lookupRaWid = useCallback(
    async (wid: string) => {
      ...
      try {
        ...
        if (res.data?.data) {
          if (!raProductName)
            setRaProductName(res.data.data.product_title || "");
        }
      } catch {
        /* not found */
      } finally {
        setRaWidLookupLoading(false);
      }
    },
    [raProductName],
  );
      ...
      if (saved) {
        ...
        if (draft.activeBox && draft.qcItems?.length > 0) {
          ...
          setTimeout(
            () =>
              toast.success(
                `Draft restored: Box ${draft.activeBox.box_id} — ${draft.qcItems.length} items`,
                { duration: 3000 },
              ),
            500,
          );
  ...
  const handleBoxScan = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.data) {
        ...
        setBoxIdInput("");
        toast.success(
          `Box ${boxId} loaded — VRP: ${res.data.data.vrp_id}, Lot: ${res.data.data.lot_type}, Qty: ${res.data.data.actual_qty}`,
        );
      ...
      toast.error(
        error?.response?.data?.error || "Box not found or not stacked",
      );
    } finally {
      setBoxLoading(false);
    }
  ...
  const handleWsnScan = useCallback(async () => {
    ...
    try {
      ...
      /* not found in master data — continue without enrichment */
    }
  ...
  const handleRaAdd = useCallback(async () => {
    ...
    if (!productName) {
      try {
        ...
        /* not found — user typed name will be used */
      }
    }
  ...
  const updateItemGrade = useCallback((id: string, grade: string) => {
    setQcItems((prev) =>
      prev.map((i) => (i._id === id ? { ...i, qc_grade: grade } : i)),
    );
  }, []);
  ...
  const handleSubmitBox = async () => {
    ...
    try {
      ...
      const data = res.data;
      toast.success(
        `Box ${activeBox.box_id} QC done — ${data.items_inserted} items. Batch: ${data.session_id}`,
      );
      ...
      setCompletedBoxes((prev) =>
        [
          {
            box_id: activeBox.box_id,
            vrp_id: activeBox.vrp_id,
            lot_type: activeBox.lot_type,
  ...
  useEffect(() => {
    if (currentTabCode === "list") loadQCList();
  }, [currentTabCode, loadQCList]);
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (entries.length === 0) {
        toast.error("No valid rows found (need BOX ID column)");
        return;
      }
      ...
      const d = res.data;
      toast.success(
        `Bulk QC: ${d.total_boxes} processed${d.error_count ? `, ${d.error_count} errors` : ""}. Batch: ${d.session_id}`,
      );
    } catch (error: any) {
      toast.error(
        "Bulk upload failed: " +
  ...
  const exportListToExcel = async () => {
    ...
    XLSX.writeFile(
      wb,
      `NL_QC_List_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  ...
  return (
    <AppLayout>
      ...
      >
        <StandardPageHeader
          title="NL QC"
          subtitle="Item-level quality check per box"
          icon="🔍"
          ...
          >
            ...
            <Paper
              sx={{
                ...
                borderRadius: 2,
                border: activeBox
                  ? `2px solid ${isDarkMode ? "#10b981" : "#059669"}`
                  : undefined,
              }}
            >
              <Stack
              ...
              >
                  ...
                  InputProps={{
                    startAdornment: (
                      <InputAdornment position="start">
                        <ScanIcon fontSize="small" color="success" />
                      </InputAdornment>
                    ),
                    endAdornment: boxLoading ? (
                ...
                {!activeBox && (
                  <Button
                    ...
                    disabled={boxLoading || !boxIdInput.trim()}
                    sx={{
                      height: 40,
                ...
                {activeBox && (
                  <>
                    ...
                    <Chip
                      icon={<BoxIcon />}
                      label={activeBox.box_id}
                      color="success"
                      variant="filled"
                      sx={{ fontWeight: 700, fontSize: "0.85rem" }}
                    />
                    <Chip
                      label={`VRP: ${activeBox.vrp_id}`}
                      size="small"
                      variant="outlined"
                      color="info"
                    />
                    <Chip
                      label={activeBox.lot_type}
                      size="small"
                      variant="filled"
                      color={
                        activeBox.lot_type === "RVP" ? "secondary" : "warning"
                      }
                      sx={{ fontWeight: 700 }}
                    />
                    <Chip
                      label={`Qty: ${activeBox.actual_qty}`}
                      size="small"
                      variant="outlined"
                    />
                    {activeBox.category && (
                      <Chip
                        label={activeBox.category}
                        size="small"
                        variant="outlined"
                      />
                    )}
                    ...
                    <Tooltip title="Close Box (discard)">
                      <IconButton
                        aria-label="Action"
                        size="small"
              ...
              <>
                ...
                {activeBox.lot_type === "RVP" && (
                  <Paper sx={{ p: 1.5, mb: 1, borderRadius: 2 }}>
                    <Typography
                      variant="subtitle2"
                      sx={{ fontWeight: 700, mb: 1, color: "#7c3aed" }}
                    >
                    ...
                    >
                      <TextField
                        ...
                        value={wsnInput}
                        onChange={(e) =>
                          setWsnInput(e.target.value.toUpperCase())
                        }
                        onKeyDown={(e) => {
                          if (e.key === "Enter") handleWsnScan();
                        }}
                        autoFocus
                        ...
                        InputProps={{
                          startAdornment: (
                            <InputAdornment position="start">
                              <ScanIcon fontSize="small" color="secondary" />
                            </InputAdornment>
                          ),
                        }}
                      />
                      <Stack direction="row" spacing={0.5}>
                        {QC_GRADES.map((g) => (
                          <Button
                            key={g}
                            variant={wsnGrade === g ? "contained" : "outlined"}
                            size="small"
                            ...
                            sx={{
                              ...
                              color: wsnGrade === g ? "white" : GRADE_COLORS[g],
                              "&:hover": {
                      ...
                      <TextField
                        ...
                        label="Remark"
                        value={wsnRemark}
                        onChange={(e) => setWsnRemark(e.target.value)}
                        sx={{ width: 140 }}
                      />
                      <Button
                        variant="contained"
                        onClick={handleWsnScan}
                        disabled={!wsnInput.trim()}
                ...
                {activeBox.lot_type === "RA" && (
                  <Paper sx={{ p: 1.5, mb: 1, borderRadius: 2 }}>
                    <Typography
                      variant="subtitle2"
                      sx={{ fontWeight: 700, mb: 1, color: "#d97706" }}
                    >
                    ...
                    >
                      <TextField
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") handleRaAdd();
                        }}
                        onBlur={() => {
                          if (raWid.trim()) lookupRaWid(raWid);
                        }}
                        autoFocus
                          ...
                          startAdornment: (
                            <InputAdornment position="start">
                              {raWidLookupLoading ? (
                                ...
                                <ScanIcon fontSize="small" color="warning" />
                              )}
                            </InputAdornment>
                          ),
                        }}
                      ...
                      <TextField
                        ...
                        label="Product Name"
                        value={raProductName}
                        onChange={(e) => setRaProductName(e.target.value)}
                        sx={{ minWidth: 150, flex: 1, maxWidth: 250 }}
                      />
                      <TextField
                        size="small"
                        label="Qty"
                        type="number"
                        value={raQty}
                        onChange={(e) => setRaQty(e.target.value)}
                        sx={{ width: 80 }}
                        inputProps={{ min: 1 }}
                      />
                      <Stack direction="row" spacing={0.5}>
                        {QC_GRADES.map((g) => (
                          <Button
                            key={g}
                            variant={raGrade === g ? "contained" : "outlined"}
                            size="small"
                            ...
                            sx={{
                              ...
                              fontWeight: 700,
                              bgcolor:
                                raGrade === g ? GRADE_COLORS[g] : "transparent",
                              borderColor: GRADE_COLORS[g],
                              color: raGrade === g ? "white" : GRADE_COLORS[g],
                              "&:hover": {
                                bgcolor: GRADE_COLORS[g],
                                color: "white",
                              },
                  ...
                  >
                    ...
                    >
                      ...
                      <Typography variant="h5" sx={{ fontWeight: 700 }}>
                        {totalItemsQcd} / {activeBox.actual_qty || "?"}
                      </Typography>
                    </CardContent>
                  </Card>
                  {activeBox.lot_type === "RA" && (
                ...
                {activeBox.actual_qty > 0 && (
                  <LinearProgress
                    variant="determinate"
                    value={Math.min(
                      (totalItemQty / activeBox.actual_qty) * 100,
                      100,
                    )}
                    color="success"
                    sx={{
                ...
                >
                  {qcItems.length === 0 ? (
                    ...
                    >
                      <Typography>
                        {activeBox.lot_type === "RVP"
                          ? "Scan WSN stickers to start QC..."
                          : "Enter WID items to start QC..."}
                      </Typography>
                    </Box>
                  ) : (
                      ...
                      >
                        <thead>
                          ...
                          >
                            ...
                            >
                              {activeBox.lot_type === "RVP" ? "WSN" : "WID"}
                            </th>
                            {activeBox.lot_type === "RA" && (
                              <th
                                style={{
                            ...
                            >
                              ...
                              >
                                <select
                                  value={item.qc_grade}
                                  onChange={(e) =>
                                    updateItemGrade(item._id, e.target.value)
                                  }
                                  style={{
                                    border: "none",
                                    ...
                                    cursor: "pointer",
                                    color:
                                      GRADE_COLORS[item.qc_grade] || "#22c55e",
                                    fontSize: "0.85rem",
                                    padding: "2px 4px",
                                  }}
                                >
                ...
                >
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Button
                      ...
                      disabled={qcItems.length === 0 || submitting}
                  ...
                  {completedBoxes.map((cb, i) => (
                    <Stack
                      ...
                      sx={{
                        ...
                        py: 0.5,
                        borderRadius: 1,
                        bgcolor: isDarkMode ? "#1a2536" : "#f8fafc",
                      }}
                    >
                      <CheckCircleIcon
                      ...
                      <Chip
                        ...
                        size="small"
                        variant="outlined"
                        color={cb.lot_type === "RVP" ? "secondary" : "warning"}
                      />
                      <Typography
                        variant="body2"
                        sx={{ color: "text.secondary" }}
              ...
              >
                ...
                >
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700, mb: 0.5 }}>
                    Scan a Box to Start QC
                  </Typography>
                  <Typography variant="body2" sx={{ color: "text.secondary" }}>
                    Scan or type a Box ID above. The system will auto-fetch VRP,
                    lot type & qty. Then QC each item based on lot type (RVP →
                    scan WSN, RA → enter WID + qty).
                  </Typography>
                </Paper>
              </Box>
            )}
          ...
          >
            ...
            >
              <TextField
                fullWidth
                size="small"
                placeholder="Search box, VRP, WSN, WID, grade..."
                value={listSearch}
        ...
        {currentTabCode === "bulk" && (
          ...
          >
            ...
            >
              ...
              >
                Upload an Excel file with box IDs and QC grades. All rows will
                be submitted as a single batch (box-level QC).
              </Typography>
              <Divider sx={{ mb: 3 }} />
              ...
              <Box sx={{ mb: 3, display: "flex", flexWrap: "wrap", gap: 0.75 }}>
                {["BOX ID *", "QC Grade", "Remarks"].map((col) => (
                  <Chip
                    ...
                    size="small"
                    variant="outlined"
                    color={col.includes("*") ? "success" : "default"}
                  />
                ))}
              </Box>
              <Stack spacing={2}>
              ...
              <Alert severity="info" sx={{ mt: 3 }}>
                This is box-level bulk QC. For item-level QC, use the QC Entry
                tab.
              </Alert>
            </Paper>
          </Box>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            QC <strong>{activeBox?.box_id}</strong> —{" "}
            <strong>
              {totalItemsQcd} item{totalItemsQcd !== 1 ? "s" : ""}
            </strong>
          ...
          {gradeBreakdown.D > 0 && (
            <Alert severity="warning" sx={{ mt: 1 }}>
              {gradeBreakdown.D} item(s) graded D — discrepancies will be
              auto-created.
            </Alert>
          )}
          <Typography variant="body2" sx={{ mt: 1, color: "text.secondary" }}>
            Box status will change to &apos;qc_done&apos;.
          </Typography>
        </DialogContent>
        <DialogActions>
