...
const getCachedInboundListData = (): any[] => {
  ...
  try {
    if (typeof window !== 'undefined') {
      ...
      const savedWarehouseId = sessionStorage.getItem('inbound_list_cache_warehouseId');
      if (saved && savedWarehouseId && parseInt(savedWarehouseId, 10) === currentWarehouseId) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          window.__INBOUND_LIST_CACHE__ = { data: parsed, total: parsed.length, timestamp: Date.now(), warehouseId: currentWarehouseId };
          return parsed;
        }
      }
    }
...
export default function InboundPage() {
      ...
      if (loadId === currentLoadIdRef.current) {
        ...
        if (typeof window !== 'undefined' && rows && rows.length > 0) {
          ...
          try {
            ...
            sessionStorage.setItem('inbound_list_cache_warehouseId', String(activeWarehouse?.id || ''));
  ...
  const handleUndo = useCallback(() => {
    ...
    if (action.type === 'paste' && action.pasteData) {
      ...
      setMultiRows(currentRows => {
        ...
        const currentSnapshot = updated.slice(startRow, endRow + 1).map(r => ({ ...r }));
  ...
  // Row-scoped refresh for performance — only refreshes affected rows instead of entire grid
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  ...
                  <Box>
                    <Typography sx={{ fontWeight: 700, fontSize: '0.9rem', color: isDarkMode ? '#e2e8f0' : '#1e293b' }}>Columns</Typography>
                    <Typography sx={{ fontSize: '0.7rem', color: isDarkMode ? '#64748b' : '#94a3b8' }}>
                      {listColumns.length} of {INBOUND_LIST_COLUMNS.length} visible
                    </Typography>
                  </Box>
                </Box>
              </AccordionSummary>
