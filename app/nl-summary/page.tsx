...
export default function NLSummaryPage() {
    ...
    const filteredData = useMemo(() => {
        ...
        return summaryData.filter((r: any) =>
    ...
    const handleExport = async () => {
        try {
            const res = await nlSummaryAPI.exportExcel({ warehouse_id: activeWarehouse?.id });
        ...
        },
        {
            ...
            cellRenderer: (p: any) => {
                const colors: Record<string, 'success' | 'warning' | 'info' | 'default'> = {
                    fully_dispatched: 'success', partially_dispatched: 'warning', available: 'info',
                };
    ...
    return (
        <AppLayout>
            ...
            <Box sx={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: isDarkMode ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' : 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)' }}>
                <StandardPageHeader title="NL Summary" subtitle="Aggregated inventory overview" icon="📊" warehouseName={activeWarehouse?.name} />
                ...
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', p: { xs: 1, sm: 1.5 } }}>
                    ...
                    <Stack direction={isMobile ? 'column' : 'row'} spacing={1.5} sx={{ mb: 1.5, flexWrap: 'wrap' }}>
                        <Card sx={{ flex: 1, minWidth: isMobile ? '100%' : 110, background: 'linear-gradient(135deg, #1e40af, #3b82f6)', color: 'white' }}>
                            <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                <Typography variant="caption" sx={{ opacity: 0.8 }}>Received</Typography>
                                <Typography variant="h5" sx={{ fontWeight: 700 }}>{Number(kpi.total_received_qty).toLocaleString()}</Typography>
                            </CardContent>
                        </Card>
                        <Card sx={{ flex: 1, minWidth: isMobile ? '100%' : 110, background: 'linear-gradient(135deg, #059669, #10b981)', color: 'white' }}>
                            <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                <Typography variant="caption" sx={{ opacity: 0.8 }}>Available</Typography>
                                <Typography variant="h5" sx={{ fontWeight: 700 }}>{Number(kpi.available_qty).toLocaleString()}</Typography>
                            </CardContent>
                        </Card>
                        <Card sx={{ flex: 1, minWidth: isMobile ? '100%' : 110, background: 'linear-gradient(135deg, #dc2626, #f87171)', color: 'white' }}>
                            <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                <Typography variant="caption" sx={{ opacity: 0.8 }}>Dispatched</Typography>
                                <Typography variant="h5" sx={{ fontWeight: 700 }}>{Number(kpi.dispatched_qty).toLocaleString()}</Typography>
                            </CardContent>
                        </Card>
                        <Card sx={{ flex: 1, minWidth: isMobile ? '100%' : 100, background: 'linear-gradient(135deg, #d97706, #f59e0b)', color: 'white' }}>
                            <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                <Typography variant="caption" sx={{ opacity: 0.8 }}>Short</Typography>
                                <Typography variant="h5" sx={{ fontWeight: 700 }}>{Number(kpi.short_qty).toLocaleString()}</Typography>
                            </CardContent>
                        </Card>
                    </Stack>
                    ...
                    <Paper sx={{ p: 1.5, mb: 1, borderRadius: 2 }}>
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ sm: 'center' }}>
                            ...
                            <Button fullWidth={isMobile} variant="outlined" size="small" startIcon={<RefreshIcon />} onClick={loadSummary} disabled={loading}>Refresh</Button>
                            <Button fullWidth={isMobile} variant="contained" size="small" startIcon={<DownloadIcon />} onClick={handleExport}>Export</Button>
                        </Stack>
                    </Paper>
                        ...
                        <Box className="ag-theme-quartz" sx={gridSx}>
                            <AgGridReact
                                ...
                                noRowsOverlayComponent={() => (
                                    <Typography sx={{ color: 'text.secondary', py: 4 }}>No records found</Typography>
                                )}
                            />
                        </Box>
                    </Box>
