...
export default function NLStackingPage() {
  ...
  const { filterTabs, canSeeTab, canSeeButton, isAdmin } =
    usePagePermissions("nl_stacking");
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const s = localStorage.getItem("nl_stacking_visible_cols");
  ...
  useEffect(() => {
    ...
    rackAPI
      .getByWarehouse(activeWarehouse.id)
      .then((res) => {
        const list = (res.data || [])
          .map((r: any) => r.rack_name || r.rack_no || r.name || String(r))
          .filter(Boolean);
        setRacks(list);
      })
      .catch(() => {});
  ...
  const lookupBox = useCallback(
    async (boxId: string, rowIndex: number) => {
      ...
      try {
        const res = await nlStackingAPI.lookupBox(
        ...
        if (res.data?.data) {
          ...
          const completeRow = {
            ...currentRow,
            box_id: boxId.trim().toUpperCase(),
            vrp_id: box.vrp_id || "",
            category: box.category || "",
            lot_type: box.lot_type || "",
            actual_qty: box.actual_qty || 0,
            _boxFound: true,
            _error: undefined,
          };
          ...
          setTimeout(() => {
            const api = gridRef.current?.api;
            if (api) {
              api.setFocusedCell(rowIndex, "rack_no");
              api.startEditingCell({ rowIndex, colKey: "rack_no" });
            }
          }, 100);
        } else {
          setRows((prev) => {
            ...
            if (u[rowIndex])
              u[rowIndex] = {
                ...u[rowIndex],
                _error: "Box not found in inbound",
              };
            return u;
          });
        }
        ...
        const errMsg =
          error?.response?.data?.error || "Box not found in inbound";
        setRows((prev) => {
          const u = [...prev];
          if (u[rowIndex]) u[rowIndex] = { ...u[rowIndex], _error: errMsg };
          return u;
        });
      } finally {
  ...
  const handleCellValueChanged = useCallback(
    (event: any) => {
      ...
      if (field === "box_id" && processedValue?.trim())
        lookupBox(processedValue, rowIndex);
  ...
  const handleSubmit = async () => {
    const filledRows = rows.filter(
      (r) => r.box_id?.trim() && r._boxFound && r.rack_no?.trim(),
    );
    if (filledRows.length === 0) {
      toast.error("No complete rows to submit (need box_id + rack_no)");
      return;
    }

    const errorRows = rows.filter((r) => r.box_id?.trim() && !r._boxFound);
    if (errorRows.length > 0) {
      toast.error(`${errorRows.length} row(s) have unresolved box IDs`);
      return;
    }

    const noRack = rows.filter(
      (r) => r.box_id?.trim() && r._boxFound && !r.rack_no?.trim(),
    );
    if (noRack.length > 0) {
      toast.error(`${noRack.length} row(s) missing rack number`);
      return;
    }
    ...
    try {
      ...
      if (data.error_count > 0) {
        toast.success(
          `Stacked ${data.total_boxes} boxes (${data.error_count} errors). Batch: ${data.session_id}`,
        );
      } else {
        toast.success(
          `Stacked ${data.total_boxes} boxes. Batch: ${data.session_id}`,
        );
      }
  ...
  const loadStackingList = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load stacking list");
    } finally {
  ...
  useEffect(() => {
    if (currentTabCode === "list") loadStackingList();
  }, [currentTabCode, loadStackingList]);
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (entries.length === 0) {
        toast.error("No valid rows found (need BOX ID + RACK NO columns)");
        return;
      }
      ...
      const d = res.data;
      toast.success(
        `Bulk stacking: ${d.total_boxes} stacked${d.error_count ? `, ${d.error_count} errors` : ""}. Batch: ${d.session_id}`,
      );
    } catch (error: any) {
      toast.error(
        "Bulk upload failed: " +
  ...
  const downloadBulkTemplate = async () => {
    const XLSX = await import("xlsx");
    const template = [
      { "BOX ID": "BOX001", "RACK NO": "R1-A-01", Remarks: "" },
    ];
  ...
  const exportListToExcel = async () => {
    ...
    const ed = listData.map((r) => ({
      ...
      "Stacking Date": formatDate(r.stacking_date),
      "Stacked By": r.stacked_by_name || "",
      "Stacked At": formatDate(r.stacked_at),
    }));
    const ws = XLSX.utils.json_to_sheet(ed);
    const wb = XLSX.utils.book_new();
    ...
    XLSX.writeFile(
      wb,
      `NL_Stacking_List_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  ...
  const columnDefs = useMemo(() => {
    ...
    visibleColumns.forEach((col: string) => {
      ...
      if (col === "rack_no") {
        ...
        colDef.cellStyle = (p: any) => {
          if (p.data._boxFound && p.value?.trim())
            return { color: "#a855f7", fontWeight: 600 };
          return null;
        };
      }
      cols.push(colDef);
    ...
    cols.push({
      ...
      cellRenderer: (params: any) => {
        if (params.data._boxFound && params.data.rack_no?.trim())
          return <CheckCircleIcon sx={{ color: "#22c55e", fontSize: 18 }} />;
        if (params.data._boxFound)
          return (
            <Tooltip title="Enter rack number">
  ...
  return (
    <AppLayout>
      ...
      >
        <StandardPageHeader
          title="NL Stacking"
          subtitle="Scan boxes to assign racks"
          icon="📦"
              ...
              >
                <Stack direction="row" spacing={1.5} alignItems="center">
                  <TextField
                    ...
                    type="date"
                    value={stackingDate}
                    onChange={(e) => setStackingDate(e.target.value)}
                    InputLabelProps={{ shrink: true }}
                    sx={{ width: 160 }}
                  />
                  <TextField
                    size="small"
                    label="Stacking Person"
                    value={stackingPerson}
                    onChange={(e) => setStackingPerson(e.target.value)}
          ...
          >
            ...
            >
              <TextField
                fullWidth
                size="small"
                placeholder="Search box ID, VRP, rack, category..."
                value={listSearch}
          ...
          >
            ...
            >
              <Typography variant="h6" sx={{ fontWeight: 700, mb: 1 }}>
                Bulk Stacking Upload
              </Typography>
              <Typography
                variant="body2"
                sx={{ color: "text.secondary", mb: 3 }}
              >
                Upload an Excel file with box IDs and rack numbers. All rows
                will be submitted as a single batch.
              </Typography>
              <Divider sx={{ mb: 3 }} />
              ...
              <Box sx={{ mb: 3, display: "flex", flexWrap: "wrap", gap: 0.75 }}>
                {["BOX ID *", "RACK NO *", "Remarks"].map((col) => (
                  <Chip
                    ...
                    size="small"
                    variant="outlined"
                    color={col.includes("*") ? "secondary" : "default"}
                  />
                ))}
              </Box>
              <Stack spacing={2}>
              ...
              <Alert severity="info" sx={{ mt: 3 }}>
                All rows will be submitted as a single batch. Box IDs must exist
                in inbound data with &apos;pending&apos; status.
              </Alert>
            </Paper>
          </Box>
        )}
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Stack <strong>{filledCount} boxes</strong> with total qty{" "}
            <strong>{totalQty}</strong> to their assigned racks?
          </Typography>
          <Typography variant="body2" sx={{ mt: 1, color: "text.secondary" }}>
            A Batch ID will be auto-generated. Box statuses will change to
            &apos;stacked&apos;.
          </Typography>
        </DialogContent>
        <DialogActions>
        ...
        <Box sx={{ overflow: "auto", flex: 1, p: 1 }}>
          ...
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Stack direction="row" spacing={1} alignItems="center">
                <ViewColumnIcon fontSize="small" color="secondary" />
