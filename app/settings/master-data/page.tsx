...
import {
  ...
  TextField, FormControl, InputLabel, Select, InputAdornment, Badge
} from '@mui/material';
import {
  ...
  Speed as SpeedIcon, Clear as ClearIcon
...
export default function MasterDataPage() {
  ...
  const loadMasterData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/master-data?${params.toString()}`,
        {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/master-data/download-template`,
        {
          method: 'GET',
          headers: {
      ...
      link.href = url;
      link.download = 'Master_Data_Template.xlsx';
  ...
  return (
    <AppLayout>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        {/* Fixed Header */}
        <AppBar position="static" elevation={2} sx={{ bgcolor: '#1976d2', color: 'white', borderBottom: 'none' }}>
          <Toolbar sx={{ minHeight: { xs: 56, sm: 64 }, px: { xs: 1.5, sm: 3 } }}>

            <Typography variant="h6" fontWeight="bold" sx={{ flexGrow: 1, fontSize: { xs: '1.1rem', sm: '1.35rem' } }}>
              📊 Master Data Management
            </Typography>
            <Stack direction="row" spacing={1} alignItems="center" sx={{ display: { xs: 'none', sm: 'flex' } }}>
              <Chip label={formatNumber(totalRecords)} color="info" size="small" variant="outlined" sx={{ height: 28, fontSize: '0.85rem', color: 'white', borderColor: 'white' }} />
              ...
              <Button color="inherit" variant="outlined" startIcon={<LogoutIcon />} onClick={handleLogout} size="small" sx={{ ml: 1, borderColor: 'white', '&:hover': { bgcolor: 'rgba(255,255,255,0.1)' } }}>
                Logout
              </Button>
            </Stack>
        ...
        <Box sx={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          {tabValue === 0 ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
              ...
              <Paper elevation={0} sx={{ p: { xs: 0.5, sm: 0.75 }, borderBottom: '2px solid #e0e0e0', bgcolor: '#fafafa' }}>
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.5} alignItems={{ xs: 'stretch', sm: 'center' }} sx={{ gap: 0.5 }}>
                  ...
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.4} alignItems={{ xs: 'stretch', sm: 'center' }} sx={{ gap: 0.4 }}>
                    {/* Upload Button */}
                    <Button variant="contained" size="small" startIcon={<UploadIcon fontSize="small" />} onClick={() => setUploadDialogOpen(true)} sx={{ height: 32, fontSize: '0.8rem', padding: '4px 10px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', '&:hover': { boxShadow: 2 }, whiteSpace: 'nowrap' }}>
                      Upload
                    </Button>

                    {/* Template Button */}
                    <Button variant="outlined" size="small" startIcon={<DownloadIcon fontSize="small" />} onClick={handleDownloadTemplate} sx={{ height: 32, fontSize: '0.8rem', padding: '4px 10px', whiteSpace: 'nowrap' }}>
                      Template
                    </Button>

                    {/* Refresh Button */}
                    <Button variant="outlined" size="small" startIcon={<RefreshIcon fontSize="small" />} onClick={loadMasterData} disabled={loading} sx={{ height: 32, fontSize: '0.8rem', padding: '4px 10px', whiteSpace: 'nowrap' }}>
                      {loading ? <CircularProgress size={16} sx={{ mr: 0.3 }} /> : null}
                      Refresh
                    </Button>

                    {/* Export Button */}
                    <Button variant="outlined" size="small" startIcon={<ExportIcon fontSize="small" />} onClick={() => setExportDialogOpen(true)} sx={{ height: 32, fontSize: '0.8rem', padding: '4px 10px', whiteSpace: 'nowrap' }}>
                      Export
                    </Button>

                    {/* Columns Button */}
                    <Button variant="outlined" size="small" startIcon={<VisibilityIcon fontSize="small" />} onClick={(e) => setColumnMenuAnchor(e.currentTarget)} sx={{ height: 32, fontSize: '0.8rem', padding: '4px 10px', whiteSpace: 'nowrap' }}>
                      Columns
                    </Button>
              ...
              {/* Advanced Filters + Reset - Collapsible */}
              {showAdvancedFilters && (
                <Paper elevation={0} sx={{ p: { xs: 0.5, sm: 0.75 }, borderBottom: '1px solid #e0e0e0', bgcolor: '#f9f9f9' }}>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.5} alignItems={{ xs: 'stretch', sm: 'center' }} sx={{ gap: 0.5 }}>
                    {/* Batch ID Filter */}
                    <FormControl size="small" sx={{ flex: { xs: 1, sm: '0 1 160px' }, minWidth: { xs: '100%', sm: '160px' } }}>
                      <InputLabel sx={{ fontSize: '0.85rem', '&.Mui-focused': { color: '#1976d2' } }}>Batch ID</InputLabel>
                      <Select
                        value={filterBatchId}
                        label="Batch ID"
                    ...
                    {/* Status Filter */}
                    <FormControl size="small" sx={{ flex: { xs: 1, sm: '0 1 140px' }, minWidth: { xs: '100%', sm: '140px' } }}>
                      <InputLabel sx={{ fontSize: '0.85rem', '&.Mui-focused': { color: '#1976d2' } }}>Status</InputLabel>
                      <Select
                        value={filterStatus}
                        label="Status"
                    ...
                    {/* Brand Filter */}
                    <FormControl size="small" sx={{ flex: { xs: 1, sm: '0 1 140px' }, minWidth: { xs: '100%', sm: '140px' } }}>
                      <InputLabel sx={{ fontSize: '0.85rem', '&.Mui-focused': { color: '#1976d2' } }}>Brand</InputLabel>
                      <Select
                        value={filterBrand}
                        label="Brand"
                    ...
                    {/* Category Filter */}
                    <FormControl size="small" sx={{ flex: { xs: 1, sm: '0 1 140px' }, minWidth: { xs: '100%', sm: '140px' } }}>
                      <InputLabel sx={{ fontSize: '0.85rem', '&.Mui-focused': { color: '#1976d2' } }}>Category</InputLabel>
                      <Select
                    ...
                    {/* Reset Button */}
                    <Button variant="outlined" size="small" startIcon={<ClearIcon fontSize="small" />} onClick={resetFilters} sx={{ height: 32, fontSize: '0.8rem', padding: '4px 10px', color: '#d32f2f', borderColor: '#d32f2f', '&:hover': { borderColor: '#b71c1c', bgcolor: '#ffebee' }, whiteSpace: 'nowrap' }}>
                      Reset
                    </Button>
                  </Stack>
                </Paper>
              ...
              {/* Last Refresh Info */}
              {/* {!showAdvancedFilters && lastRefreshTime && (
                <Box sx={{ px: { xs: 0.5, sm: 0.75 }, py: 0.25, bgcolor: '#f9f9f9', borderBottom: '1px solid #e0e0e0', display: 'flex', justifyContent: 'flex-end' }}>
                  <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.7rem' }}>🔄 {getRelativeTime(lastRefreshTime.toISOString())}</Typography>
                </Box>
              )} */}
