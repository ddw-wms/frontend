...
import {
  ...
  RemoveRedEye as ViewOnlyIcon,
  AdminPanelSettings as FullAccessIcon,
...
const PAGE_LABELS: Record<string, string> = {
  ...
  // NL (Non-Large) modules
  nl_fkt_os: "18. NL FKT OS",
  nl_inbound: "19. NL Inbound",
  nl_stacking: "20. NL Stacking",
  nl_qc: "21. NL QC",
  nl_dispatch: "22. NL Dispatch",
  nl_picking: "23. NL Picking",
  nl_summary: "24. NL Summary",
  nl_billing: "25. NL Billing",
  nl_discrepancy: "26. NL Discrepancy",
  // FSN Scanning module
  fsn_scanning: "27. FSN Scanning",
};

// Merge duplicate pages into single groups
const mergePagePermissions = (
  perms: Permission[],
...
export default function PermissionsPage() {
  const { isAdmin, refreshPermissions, canAccessMenu, canSee } =
    usePermissions();
  ...
  const TAB_WAREHOUSE_ACCESS = canSeeWarehouseAccess
    ? TAB_MY_REQUESTS + (canSeeUserOverrides ? 2 : 1)
    : -1;
  ...
  const [defaultWarehouseId, setDefaultWarehouseId] = useState<number | null>(
    null,
  );
  ...
  const handleAccordionChange =
    (page: string) => (event: React.SyntheticEvent, isExpanded: boolean) => {
    ...
    try {
      ...
      setAllPermissions(
        permRes.status === "fulfilled"
          ? permRes.value.data?.permissions || []
          : [],
      );
      setUsers(
      ...
      results.forEach((r, i) => {
        if (r.status === "rejected") {
          console.warn(
            `[Permissions] Failed to load ${names[i]}:`,
  ...
  const handleSelectAllChanges = () => {
    const pendingIds = requestDetails
      .filter((d) => d.is_approved === null)
      .map((d) => d.id);
  ...
  const handleApproveSelected = async () => {
    ...
    try {
      setLoadingApproval(true);
      const changes = Array.from(selectedChanges).map((id) => ({
        detailId: id,
        is_approved: true,
      }));
  ...
  const handleFinalizeRequest = async (
    ...
    try {
      ...
      showSnackbar(
        `Request ${action === "approve" ? "approved" : action === "reject" ? "rejected" : "finalized"}`,
        "success",
      );
  ...
  const handleSelectAllEnabled = (enabled: boolean) => {
    setRolePermissions((prev) =>
      prev.map((p) => ({ ...p, is_enabled: enabled })),
    );
    setHasUnsavedChanges(true);
  };
  ...
  const handleSelectAllVisible = (visible: boolean) => {
    setRolePermissions((prev) =>
      prev.map((p) => ({ ...p, is_visible: visible })),
    );
    setHasUnsavedChanges(true);
  };
  ...
  const getChangedPermissionsCount = (): number => {
    ...
    return rolePermissions.filter((p, idx) => {
      ...
      return (
        p.is_enabled !== orig.is_enabled || p.is_visible !== orig.is_visible
      );
    }).length;
  };
  ...
  const handleSaveRolePermissions = async () => {
    ...
    try {
      ...
      showSnackbar(
        "Permission change request submitted for approval",
        "success",
      );
      setHasUnsavedChanges(false);
      // Reset to original
  ...
  const handleSelectUser = async (user: User) => {
    ...
    try {
      ...
      setDefaultWarehouseId(
        userWh.find((w: any) => w.is_default)?.warehouse_id || null,
      );
    } catch (error) {
  ...
  const handleToggleUserOverrideEnabled = (code: string) => {
    setUserOverrides((prev) =>
      prev.map((p) => {
        ...
        return {
          ...p,
          override_enabled: newVal,
          effective_enabled: newVal ?? p.role_enabled,
        };
      }),
    );
  };
  ...
  const handleToggleUserOverrideVisible = (code: string) => {
    setUserOverrides((prev) =>
      prev.map((p) => {
        ...
        return {
          ...p,
          override_visible: newVal,
          effective_visible: newVal ?? p.role_visible,
        };
      }),
    );
  };
  ...
  const getOverrideCount = (): number => {
    return userOverrides.filter(
      (p) => p.override_enabled !== null || p.override_visible !== null,
    ).length;
  };
  ...
  const handleSaveUserOverrides = async () => {
    ...
    try {
      setSaving(true);
      await permissionsAPI.createUserOverrideRequest(
        selectedUser.id,
        overrides,
      );
      showSnackbar("Override change request submitted for approval", "success");
      // Reload original overrides
      const overridesRes = await permissionsAPI.getUserOverrides(
        selectedUser.id,
      );
      setUserOverrides(overridesRes.data.permissions || []);
      await loadApprovalData();
  ...
  const handleSaveUserWarehouses = async () => {
    ...
    if (!isSuperAdmin) {
      showSnackbar(
        "Only Super Admin can update warehouse access. Please contact your super admin.",
        "error",
      );
      return;
    }
  ...
  const RoleList = () => {
    ...
    return (
      <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
        <Typography
          variant="subtitle2"
          sx={{ px: 2, py: 1.5, bgcolor: "grey.100", fontWeight: 600 }}
        >
          Select Role
        </Typography>
            ...
            >
              <Avatar
                sx={{
                  ...
                  mr: 1.5,
                  bgcolor:
                    selectedRole?.id === role.id ? "primary.main" : "grey.300",
                  fontSize: "0.875rem",
                }}
              >
            ...
            >
              <Avatar
                sx={{
                  ...
                  mr: 1.5,
                  bgcolor:
                    selectedUser?.id === user.id ? "primary.main" : "grey.300",
                  fontSize: "0.75rem",
                }}
              >
          ...
          <Stack direction="column" spacing={1}>
            ...
            <Box
              sx={{
                ...
                gridTemplateColumns: {
                  xs: "repeat(2, 1fr)",
                  sm: "repeat(4, auto) 1fr repeat(2, auto)",
                },
                gap: { xs: 0.5, sm: 1 },
                alignItems: "center",
              }}
                    ...
                    {pageAllEnabled && (
                      <Chip
                        ...
                        sx={{
                          ...
                          "& .MuiChip-label": { px: { xs: 0.3, sm: 0.5 } },
                        }}
                      />
                    )}
                    {pageAllVisible && (
                      <Chip
                        label={isSmall ? "✓V" : "✓ Visible"}
                        size="small"
                        color="info"
                        variant="outlined"
                        sx={{
              ...
              <AccordionDetails sx={{ p: 0 }}>
                ...
                <TableContainer>
                  <Table size="small" sx={{ tableLayout: "fixed" }}>
                    <TableHead>
                      <TableRow
                        sx={{ bgcolor: isDarkMode ? "#334155" : "grey.100" }}
                      >
                        <TableCell
                          sx={{
                            fontWeight: 600,
  ...
  const UserOverrideAccordions = () => {
    ...
    // Calculate totals
    const totalEnabled = userOverrides.filter(
      (p) => p.effective_enabled,
    ).length;
    const totalVisible = userOverrides.filter(
      (p) => p.effective_visible,
    ).length;
            ...
            >
              ...
              <Tooltip title="Full Access: Override enable & show everything">
                <Button
                  ...
                  startIcon={!isSmall && <FullAccessIcon />}
                  onClick={() => {
                    handleOverrideAllEnabled(true);
                    handleOverrideAllVisible(true);
                  }}
                  sx={{
                    fontSize: { xs: "0.65rem", sm: "0.75rem" },
                    px: { xs: 0.5, sm: 1 },
                  ...
                  <Table size="small" sx={{ tableLayout: "fixed" }}>
                    ...
                    <TableBody>
                      {grouped[page].map((p) => {
                        ...
                        const enableLabel =
                          p.override_enabled === null ||
                          p.override_enabled === undefined
                            ? p.role_enabled
                              ? "Role: ON"
                              : "Role: OFF"
                            : p.override_enabled
                        ...
                        const visibleLabel =
                          p.override_visible === null ||
                          p.override_visible === undefined
                            ? p.role_visible
                              ? "Role: ON"
                              : "Role: OFF"
                            : p.override_visible
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <Tooltip
                                title={`Click to cycle: Role Default → Override ON → Override OFF → Role Default. Current: ${enableLabel}`}
                                arrow
                              >
                                <Box
                                  sx={{
                                ...
                                >
                                  <Checkbox
                                    ...
                                    onChange={() =>
                                      handleToggleUserOverrideEnabled(p.code)
                                    }
                                    color={
                                      p.override_enabled === true
                                        ? "success"
                                        : p.override_enabled === false
                                          ? "error"
                                          : "default"
                                    }
                                    size="small"
                            ...
                            >
                              <Tooltip
                                title={`Click to cycle: Role Default → Override ON → Override OFF → Role Default. Current: ${visibleLabel}`}
                                arrow
                              >
                                <Box
                                  sx={{
                                ...
                                >
                                  <Checkbox
                                    ...
                                    onChange={() =>
                                      handleToggleUserOverrideVisible(p.code)
                                    }
                                    color={
                                      p.override_visible === true
                                        ? "success"
                                        : p.override_visible === false
                                          ? "error"
                                          : "default"
                                    }
                                    size="small"
  ...
  const canAccessPermissionsPage =
    isAdmin || canAccessMenu("settings:permissions");
              ...
              >
                {selectedRole ? (
                  <>
                    ...
                    <Box
                      sx={{ flex: 1, overflow: "auto", p: { xs: 0.5, sm: 2 } }}
                    >
              ...
              >
                ...
                >
                  <Box
                    sx={{
                      p: 2,
                      bgcolor: isDarkMode ? "#1e293b" : "grey.100",
                      borderBottom: "1px solid",
                      borderColor: "divider",
                    }}
                  >
                            ...
                            primary={
                              ...
                              >
                                <Typography
                                  ...
                                  fontSize="0.875rem"
                                >
                                  {req.request_type === "role"
                                    ? `Role: ${req.role_name}`
                                    : `User: ${req.target_full_name || req.target_username}`}
                                </Typography>
                              </Stack>
                            }
                            secondary={
                              <Stack spacing={0.5}>
                                ...
                                <Stack direction="row" spacing={0.5}>
                                  <Chip
                                    label={`${req.total_changes} changes`}
                                    size="small"
                        ...
                        >
                          <Box>
                            ...
                            >
                              <Chip
                                label={
                                  selectedRequest.request_type === "role"
                                    ? `Role: ${selectedRequest.role_name}`
                                    : `User: ${selectedRequest.target_full_name}`
                                }
                                color="primary"
                                size="small"
                              />
                      ...
                      >
                        ...
                        >
                          ...
                          <Button
                            ...
                            disabled={
                              selectedChanges.size === 0 || loadingApproval
                            }
                          >
                                  ...
                                  >
                                    <TableCell padding="checkbox">
                                      <Checkbox
                                        ...
                                        onChange={() =>
                                          handleToggleChangeSelection(detail.id)
                                        }
                                    ...
                                    <TableCell align="center">
                                      ...
                                      >
                                        <Chip
                                          label={
                                            detail.old_is_enabled ? "ON" : "OFF"
                                          }
                                          size="small"
                                          color={
                                            detail.old_is_enabled
                                              ? "success"
                                              : "default"
                                          }
                                          sx={{
                                        ...
                                        <Chip
                                          label={
                                            detail.new_is_enabled ? "ON" : "OFF"
                                          }
                                          size="small"
                                          color={
                                            detail.new_is_enabled
                                              ? "success"
                                              : "default"
                                          }
                                          variant={
                        ...
                        {myRequests.map((req) => (
                          <TableRow key={req.id} hover>
                            ...
                            <TableCell>
                              {req.request_type === "role"
                                ? req.role_name
                                : req.target_full_name || req.target_username}
                            </TableCell>
                    ...
                    >
                      {selectedUser
                        ? isSmall
                          ...
                          : `Selected: ${selectedUser.full_name || selectedUser.username}`
                        : "Select User"}
                    </Button>
                    <Dialog
                      open={mobileDrawerOpen}
                      ...
                      maxWidth="xs"
                      PaperProps={{
                        sx: { borderRadius: 2, maxHeight: "70vh" },
                      }}
                    >
                      <Box
                        sx={{
                ...
                >
                  {selectedUser ? (
                      ...
                      <Box sx={{ flex: 1, overflow: "auto", p: 2 }}>
                        <Alert severity="info" sx={{ mb: 2 }}>
                          <Typography variant="body2">
                            <strong>No warehouses selected</strong> = User can
                            access ALL warehouses.
                            <br />
                            <strong>Select specific warehouses</strong> = User
                            can ONLY access selected warehouses.
                            <br />
                            <strong style={{ color: "#ed6c02" }}>
                              Note:
                            </strong>{" "}
                            User must <strong>logout and login again</strong>{" "}
                            ...
                            <Grid size={{ xs: 12, sm: 6, md: 4 }} key={wh.id}>
                              <Card
                                ...
                                sx={{
                                  cursor: "pointer",
                                  transition: "all 0.2s",
                                  borderColor: userWarehouses.includes(wh.id)
                                    ? "primary.main"
                                    : "divider",
                                  bgcolor: userWarehouses.includes(wh.id)
                                    ? alpha(theme.palette.primary.main, 0.05)
                                    : "white",
                                  "&:hover": {
                        ...
                        {userWarehouses.length > 0 && (
                          <Alert severity="success" sx={{ mt: 2 }}>
                            User has access to{" "}
                            <strong>{userWarehouses.length}</strong>{" "}
                            warehouse(s).
                            {defaultWarehouseId && (
                              <span>
                                {" "}
                    ...
                    >
                      ...
                      />
                      <Typography>
                        Select a user to manage warehouse access
                      </Typography>
                    </Box>
                  )}
                </Paper>
