...
const getCachedQCListData = (): QCItem[] => {
  ...
  if (typeof window !== "undefined" && window.__QC_LIST_CACHE__?.data?.length) {
    ...
    if (
      window.__QC_LIST_CACHE__.warehouseId === currentWarehouseId &&
...
export default function QCPage() {
  ...
  const singleQCDateInputRef = useRef<HTMLInputElement | null>(null);
  const multiQCDateInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const pageCacheRef = useRef<
    Map<string, { data: QCItem[]; total: number; timestamp: number }>
  >(new Map());
  ...
  const [enableSorting, setEnableSorting] = useState<boolean>(() => {
    try {
      return localStorage.getItem("qc_enableSorting") === "true";
    } catch {
      return false;
    }
  });
  const [enableColumnFilters, setEnableColumnFilters] = useState<boolean>(
    () => {
      try {
        return localStorage.getItem("qc_enableColumnFilters") === "true";
      } catch {
        return false;
      }
    },
  ...
  const [enableColumnResize, setEnableColumnResize] = useState<boolean>(() => {
    try {
      return localStorage.getItem("qc_enableColumnResize") !== "false";
    } catch {
      return true;
    }
  });
  ...
  const [existingQCWSNs, setExistingQCWSNs] = useState<
    Array<{ wsn: string; warehouseid: number }>
  >([]);
  ...
  const saveDraftImmediate = async (rowsToSave = multiRows) => {
    ...
    if (!hasAnyData && (draftExists || draftLoadFailedRef.current)) {
      console.warn(
        "[DRAFT] 🛡️ Blocked: refusing to overwrite QC (draftExists:",
        draftExists,
        "loadFailed:",
    ...
    try {
      ...
      console.error(
        "Failed to save QC draft to DB, falling back to IndexedDB",
        err,
      );
  ...
  const flushSyncRows = useCallback(() => {
    ...
    qcAPI.syncRows(rows, activeWarehouse.id).catch(() => {});
  }, [activeWarehouse?.id]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
        ...
        if (dbDraft?.exists && dbDraft.draft?.rows?.length > 0 && mounted) {
          const restored = dbDraft.draft.rows.map((r: any, idx: number) => {
            ...
            return {
              _rowId:
                r._rowId ||
                `qc_restored_${rowIdCounterRef.current}_${Date.now()}`,
      ...
      try {
        ...
        if (draft && draft.rows && draft.rows.length > 0 && mounted) {
          const restored = draft.rows.map((r: any, idx: number) => {
            ...
            return {
              _rowId:
                r._rowId || `qc_local_${rowIdCounterRef.current}_${Date.now()}`,
              qcdate: r.qcdate || commonQcDate,
              qcbyname: r.qcbyname || commonQcByName,
              ...r,
            };
      ...
      if (!anyLoadSucceeded) {
        ...
        console.error(
          "⚠️ QC draft load failed from both DB and IndexedDB — autosave blocked for empty rows",
        );
      }
  ...
  const [qcSettingsPanelExpanded, setQcSettingsPanelExpanded] = useState<
    string | false
  >("filters");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("qc_enableSorting", String(enableSorting));
    } catch {}
  }, [enableSorting]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem("qc_enableColumnResize", String(enableColumnResize));
    } catch {}
  }, [enableColumnResize]);
  ...
  const handleRedo = useCallback(() => {
    ...
    if (
      (action.type === "paste" || action.type === "fillDown") &&
      action.pasteData
    ) {
      ...
      gridRef.current?.refreshCells({ force: true });
      toast.success(
        `${action.type === "paste" ? "Paste" : "Fill down"} redone`,
        { duration: 1500 },
      );
    } else if (action.type === "cell") {
      if (action.rowIndex !== undefined && action.field) {
  ...
  const handleFillDown = useCallback(() => {
    ...
    if (!range) {
      toast("Select a range first (Shift+Arrow or drag)", {
        icon: "ℹ️",
        duration: 2000,
      });
      return;
  ...
  useEffect(() => {
    ...
    const handlePaste = async (e: ClipboardEvent) => {
      ...
      const newRowsSnapshot = currentRows
        .slice(startRow, endRow + 1)
        .map((r) => ({ ...r }));
      ...
      if (wsnsToPrefetch.length > 0) {
        ...
        if (gridDupWSNs.size > 0) {
          ...
          if (removedCount > 0) {
            toast.error(
              `Removed ${gridDupWSNs.size} duplicate WSN(s) from pasted data`,
              {
                duration: 5000,
      ...
      if (wsnsToFetch.length > 0) {
        toast.loading(`Fetching data for ${wsnsToFetch.length} WSN(s)...`, {
          id: "paste-fetch",
        });
        ...
        for (let i = 0; i < wsnsToFetch.length; i += BATCH_SIZE) {
          ...
          const results = await Promise.allSettled(
            batch.map(async (wsn) => {
              try {
                // Try cache first
                const { getAvailableByWSNFast } =
                  await import("@/lib/wmsCache");
                const cached = await getAvailableByWSNFast(
                  wsn,
                ...
                // Fallback to API
                const res = await qcAPI.getPendingInbound(
                  activeWarehouse?.id,
                  wsn,
                );
                const items = res.data?.data || res.data || [];
                const match = items.find?.(
                  (item: any) => item.wsn?.toUpperCase() === wsn.toUpperCase(),
                );
                return match
                  ? { wsn, data: match, source: "api" }
                  : { wsn, data: null, source: "api" };
              } catch {
        ...
        toast.success(
          `Pasted ${clipRows.length} row(s), fetched data for ${fetchedCount} WSN(s)`,
          { duration: 3000 },
        );
  ...
  const printRowWSN = useCallback(
    async (rowData: any) => {
      ...
      if (!agentReady) {
        toast.error(
          "Print agent is not running. Please start the WMS Print Agent app.",
          { duration: 4000 },
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        (ctrlKey && e.key.toLowerCase() === "y" && !isEditing) ||
        (ctrlKey && shiftKey && e.key.toLowerCase() === "z" && !isEditing)
      ) {
        e.preventDefault();
        handleRedo();
        return;
      ...
      if (ctrlKey && e.key.toLowerCase() === "o" && !isEditing) {
        ...
        if (fktLink) {
          const url = fktLink.startsWith("http")
            ? fktLink
            : `https://${fktLink}`;
          ...
          if (colIdx > 0) {
            ...
            if (currentHasData) {
              ...
              if (hasCellData(currentRow, prevColId)) {
                let c = colIdx - 1;
                while (c - 1 >= 0 && hasCellData(currentRow, colIds[c - 1]))
                  c--;
                targetColId = colIds[c];
              } else {
                let found = false;
  ...
  const fetchProductDetails = async (wsn: string) => {
    ...
    try {
      ...
      const pendingResponse = await qcAPI.getPendingInbound(
        activeWarehouse?.id,
        upperWsn,
      );
  ...
  const listColumnDefs = useMemo(() => {
    const sr = {
      ...
      valueGetter: (params: any) =>
        params.node ? params.node.rowIndex + 1 + (page - 1) * limit : undefined,
      width: 20,
      cellStyle: {
        fontWeight: 700,
        textAlign: "center",
  ...
  const loadQCList = async ({ buttonRefresh = false } = {}) => {
    ...
    if (qcAbortControllerRef.current) {
      try {
        qcAbortControllerRef.current.abort();
      } catch {}
            ...
            try {
              ...
              if (
                ...
                const prevMap = new Map(
                  previousDataRef.current.map((r: any) => [r.id || r.wsn, r]),
                );
                const currMap = new Map(
                  data.map((r: any) => [r.id || r.wsn, r]),
                );
    ...
    try {
      ...
      if (error.response?.status === 409) {
        ...
        if (data.canUpdate === false) {
          toast.error(
            "WSN already exists in another warehouse - cannot update",
          );
          setDuplicateQC(null);
        } else {
          setDuplicateQC(data);
          toast.error(
            'Duplicate QC in this warehouse - Click "Update" to modify',
          );
        }
      } else {
  ...
  const handleConfirmDownload = async () => {
    try {
      ...
      const response = await qcAPI.downloadValidatedTemplate(
        activeWarehouse?.id || 0,
      );
  ...
  const exportMultiEntryToExcel = async (selectedCols: string[]) => {
    try {
      ...
      const columnsToExport = EXPORT_COLUMN_MAP.filter((c) =>
      ...
      XLSX.writeFile(
        wb,
        `QC_MultiEntry_${new Date().toISOString().split("T")[0]}_${Date.now()}.xlsx`,
      );
      toast.success(
        `✓ Exported ${dataToExport.length} rows with ${columnsToExport.length} columns`,
      );
    } catch (error) {
    ...
    const cols = visibleColumns.map((field: string) => {
      ...
      const baseColDef: any = {
        ...
        headerName:
          field === "sno"
            ? "S.No"
            : String(field).replace(/_/g, " ").toUpperCase(),
  ...
  const handleOverwriteReplace = useCallback(async () => {
    ...
    try {
      const response = await qcAPI.getPendingInbound(
        activeWarehouse?.id,
        newWSN,
      );
  ...
  const handleMultiSubmit = async () => {
    ...
    const hasGridDuplicates = Array.from(wsnCounts.values()).some(
      (count) => count > 1,
    );
    ...
    if (crossWarehouseFound.length > 0) {
      toast.error(
        `${crossWarehouseFound.length} WSN(s) already QC'd in other warehouse`,
      );
      return;
    }
    ...
    if (sameWarehouseFound.length > 0) {
      toast.error(
        `${sameWarehouseFound.length} WSN(s) already QC'd in this warehouse`,
      );
      return;
    }
    ...
    try {
      ...
      if (successCount === totalCount) {
        ...
        toast.success(
          `✓ ${successCount}/${totalCount} entries created. ${totalCount - successCount} rows kept for review.`,
        );
        setMultiRows((prevRows) => {
          ...
          return emptyNeeded > 0
            ? [...remaining, ...generateEmptyRows(emptyNeeded)]
            : remaining;
        });
      ...
      toast.error(
        error.response?.data?.error ||
          "Submission failed. Data preserved — retry when ready.",
      );
    } finally {
  ...
  const handleMultiCellValueChanged = useCallback(
    (event: any) => {
      ...
      if (field === "wsn") {
        ...
        const existingRecord = existingQCWSNs.find((item) => item.wsn === wsn);
        const isSameWarehouseDup =
          existingRecord?.warehouseid === activeWarehouse?.id;
        const isCrossWarehouse =
          existingRecord && existingRecord.warehouseid !== activeWarehouse?.id;
        ...
        if (wsn) {
          ...
          if (!isOnline && !offlineWarningShownRef.current) {
            ...
            toast(
              "📴 Offline Mode - Using cached data. Will validate on submit.",
              {
                duration: 5000,
                style: {
                  background: "#fef3c7",
          ...
          (async () => {
            try {
              ...
              if (isWMSCacheEnabled() && activeWarehouse?.id) {
                try {
                  masterInfo = await getAvailableByWSNFast(
                    wsn,
                    activeWarehouse.id,
                  );
                  if (masterInfo) {
              ...
              if (latestWSN !== wsn) {
                console.log(
                  `⏭️ Skipping stale fetch for row ${rowIndex}: ${wsn} (latest: ${latestWSN})`,
                );
                return;
              }
  ...
  useRealtimeSync({
    ...
    onDataSubmitted: useCallback(
      (data: any) => {
        toast.success(
          `${data.submittedBy} submitted ${data.successCount} QC entries from another device`,
          { duration: 4000, icon: "📡" },
        );
        // Add submitted WSNs to existing list for duplicate prevention
        if (data.submittedWSNs?.length) {
          setExistingQCWSNs((prev: any[]) => {
            const newEntries = data.submittedWSNs!.map((wsn: string) => ({
              wsn,
              warehouseid: activeWarehouse?.id,
            }));
    ...
    onDraftUpdated: useCallback((data: any) => {
      ...
      toast("QC draft updated from another device", {
        duration: 3000,
        icon: "📝",
      });
    }, []),
    onDraftCleared: useCallback(() => {
      toast("QC draft cleared from another device", {
        duration: 3000,
        icon: "🗑️",
      });
      ...
      >
            ...
            >
              ...
              >
                  ...
                  (!qcList || qcList.length === 0) && (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          No QC items match your current filters. Try adjusting
                          your search criteria or reset filters to see all
                          items.
                        </Typography>
                      </Box>
                    </Box>
                ...
                >
                    ...
                    >
                      <AgGridReact
                        ...
                        onGridReady={(params: any) => {
                          ...
                          try {
                            const savedState =
                              localStorage.getItem("qc_list_grid_state");
                  ...
                  {((listLoading && (!qcList || qcList.length === 0)) ||
                    (qcList && qcList.length > 0 && !gridDataRendered)) && (
                    <Box
                      sx={{
                        position: "absolute",
                        top: 32,
              ...
              <Fade in={true} timeout={300}>
                ...
                >
                  ...
                  >
                    {qcList.length > 0 ? (page - 1) * limit + 1 : 0} –{" "}
                ...
                >
                  ...
                  >
                    ...
                    {duplicateQC && (
                      ...
                      >
                        ⚠️ QC already exists. Click "Update" to modify existing
                        entry.
                      </Alert>
                    )}
                    ...
                    <Box sx={{ flex: 1, overflow: "auto" }}>
                      <Stack spacing={1.5}>
                        ...
                        <TextField
                          ...
                          inputRef={singleQCDateInputRef}
                          onClick={() => {
                            try {
                              singleQCDateInputRef.current?.showPicker?.();
                            } catch {
                              singleQCDateInputRef.current?.focus();
                            }
                          }}
                          sx={{
                            "& .MuiOutlinedInput-root": {
                              ...
                              <Box>
                                ...
                                <Typography
                                  sx={{
                                    ...
                                    "&:hover": {
                                      color: isDarkMode ? "#7dd3fc" : "#065f46",
                                    },
                                  }}
                                  onClick={() =>
                                    window.open(
                ...
                >
                      ...
                      >
                        ...
                        >
                          <TextField
                            ...
                            inputRef={multiQCDateInputRef}
                            onClick={() => {
                              try {
                                multiQCDateInputRef.current?.showPicker?.();
                              } catch {
                                multiQCDateInputRef.current?.focus();
                              }
                            }}
                            sx={{
                              "& .MuiInputBase-root": {
                  ...
                  >
                    ...
                    >
                      ...
                      <IconButton
                        size="small"
                        onClick={() => setQcSettingsPanelOpen(false)}
                        sx={{ color: "white" }}
                      >
                        <CloseIcon />
                      </IconButton>
                    ...
                    <Box sx={{ overflow: "auto", flex: 1 }}>
                      ...
                      >
                        ...
                        <AccordionDetails sx={{ px: 2, pb: 2, pt: 0 }}>
                          <Stack spacing={1.5}>
                            ...
                            >
                              Cache available inventory for instant WSN lookup
                              during QC.
                            </Alert>
                            <Box
                              sx={{
                            ...
                            <Button
                              ...
                              variant="contained"
                              disabled={
                                availableCacheLoading || !activeWarehouse?.id
                              }
                      ...
                      >
                        <Button
                          ...
                          onClick={() => {
                            setQcSettingsPanelOpen(false);
                            setExportColumnsDialogOpen(true);
                          }}
                      ...
                      <Box sx={{ p: 2 }}>
                        ...
                        >
                          <Box
                            sx={{
                              ...
                              height: 8,
                              borderRadius: "50%",
                              bgcolor: agentReady ? "#10b981" : "#ef4444",
                            }}
                          />
                          <Typography
                            variant="caption"
                          ...
                          >
                            Print Agent:{" "}
                            {agentReady ? "Connected" : "Not Running"}
                          </Typography>
                        </Box>
                        ...
                        >
                          Enable shortcuts to quickly print labels or open
                          product pages after scanning a WSN.
                        </Typography>
                      </Box>
                    </Box>
                  </Drawer>
                ...
                <AgGridReact
                  ...
                  onColumnResized={(params: any) => {
                    if (
                      ...
                      if (colId === "print_action" || colId === "_filler")
                        return;
                      setMultiColumnWidths((prev) => {
                        const updated = { ...prev, [colId]: newWidth };
                        localStorage.setItem(
                          "qcMultiEntryColumnWidths",
                            ...
                            onChange={(e) => {
                              ...
                              const ordered = [
                                ...
                                ...EDITABLE_COLUMNS.filter(
                                  (c) => c !== "sno" && next.includes(c),
                                ), // User-selected editable columns
                                ...ALL_MASTER_COLUMNS.filter((c) =>
                                  next.includes(c),
                                ), // User-selected master columns
                              ];
            ...
            <Stack spacing={3}>
              ...
              >
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mt: 1 }}>
                  ...
                  {(exportGrade || gradeFilter) && (
                    <Chip
                      size="small"
                      label={`📊 Grade: ${exportGrade || gradeFilter}`}
                      sx={{
                        bgcolor: isDarkMode
                          ? "rgba(34, 197, 94, 0.2)"
                          : "#dcfce7",
                  ...
                  {(exportBrand || brandFilter) && (
                    <Chip
                      size="small"
                      label={`🏷️ ${exportBrand || brandFilter}`}
                      sx={{
                        bgcolor: isDarkMode
                          ? "rgba(245, 158, 11, 0.2)"
                          : "#fef3c7",
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                      {(selected as string[]).map((value) => (
                        <Chip
                          ...
                          onDelete={() =>
                            setExportBatchIds(
                              exportBatchIds.filter((id) => id !== value),
                            )
                          }
              ...
              >
                <CardContent sx={{ py: 2, px: 3 }}>
                  ...
                  >
                    This will export filtered QC records to Excel with all
                    selected criteria applied. The file will include QC details,
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 2 }}>
              <Stack direction="row" spacing={1}>
                <TextField
                  label={
                    editingGradeIndex !== null ? "Edit Grade" : "New Grade"
                  }
                  value={newGrade}
              ...
              {ALL_LIST_COLUMNS.map((col) => (
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      checked={
                        listColumns.find((c) => c.key === col)?.visible || false
                      }
        ...
        >
          ...
          <DialogContent sx={{ pt: "16px !important" }}>
            ...
            >
              {EXPORT_COLUMN_MAP.filter(
                (c) => c.key === "sno" || !ALL_MASTER_COLUMNS.includes(c.key),
              ).map((col) => (
                <FormControlLabel
                  key={col.key}
                  control={
            ...
            >
              {EXPORT_COLUMN_MAP.filter((c) =>
                ALL_MASTER_COLUMNS.includes(c.key),
              ).map((col) => (
                <FormControlLabel
                  key={col.key}
                  control={
            ...
            >
              <FormControlLabel
                ...
                label={
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>
                    Save column selection for next time
                  </Typography>
                }
              />
            </Box>
          ...
          <DialogActions sx={{ px: 3, pb: 2 }}>
            <Button
              onClick={() => setExportColumnsDialogOpen(false)}
        ...
        >
          ...
          >
            ...
            <IconButton
              size="small"
              onClick={() => setQcOptionsPanelOpen(false)}
              sx={{ color: "white" }}
            >
              <CloseIcon />
            </IconButton>
          ...
          <Box sx={{ overflow: "auto", flex: 1 }}>
            ...
            <Accordion
              ...
              onChange={(_, isExpanded) =>
                setQcSettingsPanelExpanded(isExpanded ? "filters" : false)
              }
              disableGutters
              sx={{
                bgcolor: "transparent",
            ...
            {canSeeButton("list:columns") && (
              <Accordion
                ...
                onChange={(_, isExpanded) =>
                  setQcSettingsPanelExpanded(isExpanded ? "columns" : false)
                }
                disableGutters
                sx={{
                  bgcolor: "transparent",
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    ...
                    <Box>
                      ...
                      >
                        {listColumns.filter((c) => c.visible).length} of{" "}
                        {ALL_LIST_COLUMNS.length} visible
                      </Typography>
                    </Box>
                  </Box>
                </AccordionSummary>
            ...
            <Accordion
              ...
              onChange={(_, isExpanded) =>
                setQcSettingsPanelExpanded(isExpanded ? "grid" : false)
              }
              disableGutters
              sx={{
                bgcolor: "transparent",
