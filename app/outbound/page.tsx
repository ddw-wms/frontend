...
const getCachedOutboundListData = (): OutboundItem[] => {
  ...
  if (
    ...
    window.__OUTBOUND_LIST_CACHE__?.data?.length
  ) {
    // Only use cache if warehouse matches and not stale (2 min TTL)
    if (
      window.__OUTBOUND_LIST_CACHE__.warehouseId === currentWarehouseId &&
  ...
  try {
    if (typeof window !== "undefined") {
      ...
      const savedWarehouseId = sessionStorage.getItem(
        "outbound_list_cache_warehouseId",
      );
      if (
        saved &&
        savedWarehouseId &&
...
export default function OutboundPage() {
  ...
  const singleDispatchDateInputRef = useRef<HTMLInputElement | null>(null);
  const multiDispatchDateInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const [cacheEnabled, setCacheEnabled] = useState<boolean>(() => {
    try {
      const val = localStorage.getItem("outbound_cacheEnabled");
  ...
  const add500Rows = () => {
    setMultiRows(
      (prev) => [...prev, ...generateEmptyRows(500)] as OutboundItem[],
    );
  };
  ...
  const EXPORT_COLUMNS_CONFIG: {
    ...
    getValue: (
      row: any,
      idx: number,
      customerForExport: string,
      vehicleForExport: string,
      dateForExport: string,
    ) => any;
  }[] = [
    ...
    { key: "wsn", label: "WSN", getValue: (r) => r.wsn || "" },
    {
      key: "dispatch_date",
      label: "Dispatch Date",
      getValue: (r, _i, _c, _v, d) => r.dispatch_date || d,
    },
    {
      key: "customer_name",
      label: "Customer Name",
      getValue: (r, _i, c) => r.customer_name || c,
    },
    { key: "vehicle_no", label: "Vehicle No", getValue: (_r, _i, _c, v) => v },
    {
      key: "product_title",
      label: "Product Title",
      getValue: (r) => r.product_title || "",
    },
    ...
    { key: "vrp", label: "VRP", getValue: (r) => r.vrp || "" },
    {
      ...
      getValue: (r) => r.dispatch_remarks || "",
    },
    {
      key: "other_remarks",
      label: "Other Remarks",
      getValue: (r) => r.other_remarks || "",
    },
    {
      key: "cms_vertical",
      label: "CMS Vertical",
      getValue: (r) => r.cms_vertical || "",
    },
    ...
    { key: "hsn_sac", label: "HSN/SAC", getValue: (r) => r.hsn_sac || "" },
    {
      key: "igst_rate",
      label: "IGST Rate",
      getValue: (r) => r.igst_rate || "",
    },
    {
      key: "yield_value",
      label: "Yield Value",
      getValue: (r) => r.yield_value || "",
    },
    {
      key: "invoice_date",
      label: "Invoice Date",
  ...
  const [preSubmitExportDialogOpen, setPreSubmitExportDialogOpen] =
    useState(false);
  ...
  const [exportRememberColumns, setExportRememberColumns] = useState(() => {
    try {
      return localStorage.getItem("outboundExportRemember") === "true";
    } catch {
      return false;
    }
  });
    ...
    try {
      ...
      if (exportRememberColumns) {
        localStorage.setItem(
          "outboundExportColumns",
          JSON.stringify(exportSelectedColumns),
        );
      ...
      const dateForExport =
        commonDate || new Date().toISOString().split("T")[0];

      // Build export data with only selected columns (preserve order from config)
      const selectedConfigs = EXPORT_COLUMNS_CONFIG.filter((c) =>
        exportSelectedColumns.includes(c.key),
      );
      const exportData = dataToExport.map((row: any, idx: number) => {
        const obj: Record<string, any> = {};
        selectedConfigs.forEach((col) => {
          obj[col.label] = col.getValue(
            row,
            idx,
            customerForExport,
            vehicleForExport,
      ...
      const safeCustomerName = (customerForExport || "Unknown")
  ...
  const [multiRows, setMultiRows] = useState<OutboundItem[]>(() =>
  ...
  const syncDispatchingWSNs = async (rowsToSync = multiRows) => {
    ...
    if (wsns.length === 0) {
      ...
      if (hasEverSyncedDispatchWSNsRef.current) {
        ...
        console.log(
          "🗑️ All WSNs removed from outbound grid — clearing dispatching_wsns from DB",
        );
  ...
  useEffect(() => {
    ...
    if (dispatchingSyncTimeoutRef.current)
      clearTimeout(dispatchingSyncTimeoutRef.current);
  ...
  const [outboundSettingsPanelOpen, setOutboundSettingsPanelOpen] =
    useState(false);
  ...
  const [pivotGroupBy, setPivotGroupBy] = useState<
    "category" | "brand" | "p_type" | "combined" | "crosstab"
  >("category");
  const [categoryPivotSortBy, setCategoryPivotSortBy] = useState<
    "category" | "qty" | "fsp" | "mrp"
  >("qty");
  const [categoryPivotSortDir, setCategoryPivotSortDir] = useState<
    "asc" | "desc"
  >("desc");
  ...
  const [crossTabRowField, setCrossTabRowField] = useState<
    "category" | "brand" | "p_type"
  >("brand");
  const [crossTabColField, setCrossTabColField] = useState<
    "category" | "brand" | "p_type"
  >("p_type");
  ...
  const [listData, setListData] = useState<OutboundItem[]>(() =>
    getCachedOutboundListData(),
  );
  const [listLoading, setListLoading] = useState(
    () => getCachedOutboundListData().length === 0,
  );
  ...
  const [enableSorting, setEnableSorting] = useState<boolean>(() => {
    try {
      return localStorage.getItem("outbound_enableSorting") === "true";
    } catch {
      return false;
    }
  });
  const [enableColumnFilters, setEnableColumnFilters] = useState<boolean>(
    () => {
      try {
        return localStorage.getItem("outbound_enableColumnFilters") === "true";
      } catch {
        return false;
      }
    },
  ...
  const [enableColumnResize, setEnableColumnResize] = useState<boolean>(() => {
    try {
      return localStorage.getItem("outbound_enableColumnResize") !== "false";
    } catch {
      return true;
    }
  });
  const [enableCellEditing, setEnableCellEditing] = useState<boolean>(() => {
    try {
      return localStorage.getItem("outbound_enableCellEditing") !== "false";
    } catch {
      return true;
    }
  });
  ...
  const pageCacheRef = useRef<
    Map<string, { data: OutboundItem[]; total: number; timestamp: number }>
  >(new Map());
    ...
    if (typeof window !== "undefined") {
      ...
      if (saved) {
        try {
          ...
          const valid = parsed.filter((col: string) =>
            ALL_LIST_COLUMNS.includes(col),
          );
          if (valid.length > 0) {
            return valid;
          }
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        const valid = cols.filter((col: string) =>
          ALL_MULTI_COLUMNS.includes(col),
        );
        if (valid.length > 0) {
  ...
  useEffect(() => {
    localStorage.setItem(
      "outbound_ctrlOProductLinkEnabled",
  ...
  const saveColumnSettings = (cols: string[]) => {
    ...
    localStorage.setItem(
      "outboundMultiEntryColumns",
      JSON.stringify(orderedCols),
    );
  };
        ...
        try {
          ...
          if (api) {
            ...
            if (typeof api.refreshCells === "function") {
              ...
              console.log("🔄 SR.NO refreshCells called with context:", {
                page,
                limit,
              });
            }
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        if (response.data.length === 0) {
          console.warn(
            "No customers found for this warehouse. Please add customers first.",
          );
        }
      } else {
        console.error(
          "Invalid response format from customerAPI:",
          response.data,
        );
        setCustomers([]);
      }
  ...
  const fetchSourceByWSN = async (wsn: string, quiet = false) => {
    ...
    try {
      ...
      setSingleForm((prev) => ({
        ...
        vehicle_no:
          data.vehicle_no || data.inbound_vehicle_no || prev.vehicle_no,
      }));
      ...
      if (err.response?.status === 409) {
        ...
        if (!quiet)
          toast.error(
            err.response?.data?.error || "Failed to fetch source data",
          );
      }
    } finally {
      ...
      try {
        ...
        if (needsRefresh) {
          setCacheProgress({
            loaded: 0,
            total: 1,
            message: "Loading available inventory...",
          });
          ...
          if (result.success) {
            toast.success(
              `✅ Cached ${result.count.toLocaleString()} available items`,
            );
          }
        }
  ...
  const refreshCache = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `✅ Cache refreshed: ${result.count.toLocaleString()} items`,
        );
        const stats = await getOutboundCacheStats(activeWarehouse.id);
        setCacheStats({
          totalRecords: stats.totalRecords,
  ...
  const handleUndo = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (api) {
        if (action.type === "batch" && action.batchChanges?.length) {
          ...
          api.setFocusedCell(
            action.batchChanges[0].rowIndex,
            action.batchChanges[0].field,
          );
        } else {
    ...
    const count =
      action.type === "batch" ? action.batchChanges?.length || 1 : 1;
    toast.success(`Undo: ${count} cell${count > 1 ? "s" : ""}`, {
      duration: 1000,
    });
  }, []);
  ...
  const handleRedo = useCallback(() => {
    ...
    const count =
      action.type === "batch" ? action.batchChanges?.length || 1 : 1;
    toast.success(`Redo: ${count} cell${count > 1 ? "s" : ""}`, {
      duration: 1000,
    });
  }, []);
  ...
  const prevSelectionBoundsRef = useRef<{
    minRow: number;
    maxRow: number;
  } | null>(null);
  ...
  const updateSelectionRange = useCallback((range: typeof selectedRange) => {
    ...
    if (api) {
      ...
      prevSelectionBoundsRef.current = bounds
        ? { minRow: bounds.minRow, maxRow: bounds.maxRow }
        : null;
  ...
  const navigateToNextCell = useCallback((params: any) => {
    ...
    if (key === "Tab") {
      ...
      const currentColIndex = allColumns.findIndex(
        (c: any) => c.getColId() === column.getColId(),
      );
      ...
      if (goingBack) {
        ...
        if (currentColIndex > 0) {
          return {
            rowIndex,
            column: allColumns[currentColIndex - 1],
            rowPinned: null,
          };
        } else if (rowIndex > 0) {
      ...
      if (pastedWSNCells.length > 0 && activeWarehouse?.id) {
        ...
        if (validUniqueWSNs.length > 0) {
          const toastId = toast.loading(
            `Loading ${validUniqueWSNs.length} WSNs...`,
          );
          ...
          for (let i = 0; i < validUniqueWSNs.length; i += LOOKUP_BATCH) {
              ...
              batch.map(async (wsn) => {
                try {
                  ...
                  if (cacheEnabled && cacheStats?.isReady) {
                    try {
                      const cached = await getAvailableInventoryByWSNFast(
                        wsn,
                        activeWarehouse!.id,
                      );
            ...
            toast.loading(
              `Loading ${Math.min(i + LOOKUP_BATCH, validUniqueWSNs.length)}/${validUniqueWSNs.length} WSNs...`,
              { id: toastId },
            );
          }
        ...
        if (allProblematic.size > 0) {
          ...
          if (alreadyDispatchedWSNs.size > 0)
            msgs.push(`${alreadyDispatchedWSNs.size} already dispatched`);
        ...
        if (noMasterDataWSNs.size > 0) {
          toast.error(
            `${noMasterDataWSNs.size} WSN(s) not found in source data — removed from grid.`,
            {
              duration: 6000,
              style: { fontWeight: 600, fontSize: "14px" },
              icon: "❌",
          ...
          toast.success(
            `✓ Source data loaded for ${masterDataMap.size} WSN${masterDataMap.size > 1 ? "s" : ""}`,
            { duration: 2000 },
          );
        }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        ...
        if (e.key === "ArrowDown") {
          ...
          if (targetRow === rowIndex)
            targetRow = multiRowsRef.current.length - 1;
      ...
      if (
        ...
        if (e.key === "ArrowDown") {
          ...
          if (endRow === (currentRange?.endRow ?? rowIndex))
            endRow = multiRowsRef.current.length - 1;
        } else if (e.key === "ArrowUp") {
          endRow = 0;
        } else if (e.key === "ArrowRight") {
      ...
      if (
        ...
        const currentRange = selectedRangeRef.current;
        let currentEndRow = currentRange
          ? currentRange.endRow
          : rangeStartCellRef.current.rowIndex;
        let currentEndCol = currentRange
          ? currentRange.endCol
          : rangeStartCellRef.current.colId;
        ...
        if (e.key === "ArrowRight") {
          const newColIndex = Math.min(
            colIds.length - 1,
            currentEndColIndex + 1,
          );
          newEndCol = colIds[newColIndex];
        }
  ...
  const lastSentHeaderRef = useRef<{
    date: string;
    customer: string;
    vehicle: string;
  ...
  useEffect(() => {
    ...
    if (
      current.date === lastSentHeaderRef.current.date &&
      current.customer === lastSentHeaderRef.current.customer &&
      current.vehicle === lastSentHeaderRef.current.vehicle
    )
      return;

    if (headerSyncTimeoutRef.current)
      clearTimeout(headerSyncTimeoutRef.current);
  ...
  const onCellValueChanged = useCallback(
    async (params: any) => {
      ...
      if (
        ...
        rowIndex !== null &&
        rowIndex !== undefined &&
        EDITABLE_COLUMNS.includes(field)
      ) {
      ...
      if (field === "wsn") {
        ...
        if (
          rowIndex != null &&
          rowIndex >= params.api.getDisplayedRowCount() - 2
        ) {
          add500Rows();
        }
          ...
          setTimeout(() => {
            try {
              ...
              if (nextIndex < params.api.getDisplayedRowCount()) {
                ...
                const firstVisible =
                  params.api.getFirstDisplayedRowIndex?.() ?? 0;
                const lastVisible =
                  params.api.getLastDisplayedRowIndex?.() ??
        ...
        try {
          ...
          toast.success(
            `✓ ${fromCache ? "(Cache)" : ""} Data loaded for ${wsn}`,
          );
        } catch (err: any) {
  ...
  const getRowStyle = useCallback(
    (params: any): { [key: string]: string } | undefined => {
      ...
      if (rowIndex !== undefined && highlightedRows.has(rowIndex)) {
        return {
          background:
            "linear-gradient(90deg, #e8f5e9 0%, #c8e6c9 50%, #e8f5e9 100%)",
  ...
  const handleOverwriteReplace = async () => {
    ...
    if (pendingWSNRef.current && activeWarehouse?.id) {
      ...
      if (rowNode) {
        ...
        try {
          const data = await getAvailableInventoryByWSNFast(
            wsn,
            activeWarehouse.id,
          );
          if (data) {
        ...
        if (rowNode) {
          ...
          setTimeout(
            () =>
              queueRowSync(
                nextEmptyIndex,
                multiRowsRef.current[nextEmptyIndex],
              ),
            50,
          );
    ...
    try {
      ...
      if (successCount === totalCount) {
        ...
        if (isWMSCacheEnabled()) {
          const dispatchedWSNs = entriesWithCommonFields
            .map((r: any) => r.wsn?.trim()?.toUpperCase())
            .filter(Boolean);
  ...
  const saveDraftImmediate = async (rowsToSave = multiRows) => {
    ...
    if (!hasAnyData && (draftExists || draftLoadFailedRef.current)) {
      console.warn(
        "[DRAFT] 🛡️ Blocked: refusing to overwrite outbound (draftExists:",
        draftExists,
        "loadFailed:",
        draftLoadFailedRef.current,
        ")",
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "Failed to save outbound draft to DB, falling back to IndexedDB",
        err,
      );
    ...
    const load = async () => {
      ...
      try {
        ...
        if (dbDraft?.exists && dbDraft.draft?.rows?.length > 0 && mounted) {
          ...
          setDraftSavedAt(
            new Date(
              dbDraft.draft.saved_at || dbDraft.draft.updated_at,
            ).getTime(),
          );
          setDraftExists(true);
      ...
      if (!anyLoadSucceeded) {
        ...
        console.error(
          "⚠️ Outbound draft load failed from both DB and IndexedDB — autosave blocked for empty rows",
        );
      }
  ...
  const loadOutboundList = async (
    opts: { buttonRefresh?: boolean } = { buttonRefresh: false },
  ) => {
    if (!activeWarehouse) return;
    ...
    if (buttonRefresh) {
      ...
      overlayTimerRef.current = setTimeout(() => {
        try {
          ...
          try {
            listGridRef.current?.api?.hideOverlay();
          } catch {}
        } catch (err) {}
    ...
    if (outboundAbortControllerRef.current) {
      try {
        outboundAbortControllerRef.current.abort();
      } catch {}
    ...
    try {
      ...
      if (loadId === currentLoadIdRef.current) {
        ...
        console.log("📦 Outbound List Data:", {
          page,
          limit,
        ...
        try {
          ...
          if (api && typeof api.setRowData === "function") {
            api.setRowData(data);
            console.log(
              "📦 AG Grid setRowData called with",
              data.length,
              "items",
            );
          }
        ...
        if (uniqCusts.length === 0) {
          const names: string[] = data
            .map((d: any) => String(d.customer_name || ""))
            .filter((n: string) => n !== "");
        ...
        try {
          ...
          if (Array.isArray(sRes.data) && sRes.data.length > 0)
            uniqSources = sRes.data;
        } catch (err) {
          // ignore and fallback to data
        }

        if (uniqSources.length === 0) {
          const srcs: string[] = data
            .map((d: any) => String(d.source || ""))
            .filter((s: string) => s !== "");
        ...
        // Calculate stats
        const pickingCount = data.filter(
          (d: any) => d.source === "PICKING",
        ).length;
        const qcCount = data.filter((d: any) => d.source === "QC").length;
        const inboundCount = data.filter(
          (d: any) => d.source === "INBOUND",
        ).length;
      ...
      if (retryCountRef.current < MAX_RETRIES && !buttonRefresh) {
        ...
        toast.error(
          `Loading failed, retrying... (${retryCountRef.current}/${MAX_RETRIES})`,
        );
        setTimeout(() => {
          loadOutboundList({ buttonRefresh: true });
      ...
      if (now - lastErrorToastRef.current > ERROR_TOAST_COOLDOWN) {
        ...
        if (buttonRefresh) {
          toast.error(
            err.response?.data?.error || "Failed to refresh outbound list",
          );
        } else {
          toast.error(
            err.response?.data?.error || "Failed to load outbound list",
          );
        }
      }
      retryCountRef.current = 0;
        ...
        if (overlayShownRef.current && overlayStartRef.current) {
          ...
          if (elapsed < MIN_LOADING_MS) {
            await new Promise((res) =>
              setTimeout(res, MIN_LOADING_MS - elapsed),
            );
          }
          try {
            listGridRef.current?.api?.hideOverlay();
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `outbound_export_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(
        "outbound_enableColumnFilters",
        String(enableColumnFilters),
      );
      localStorage.setItem(
        "outbound_enableColumnResize",
        String(enableColumnResize),
      );
      localStorage.setItem(
        "outbound_enableCellEditing",
        String(enableCellEditing),
      );
    } catch {}
  }, [
    enableSorting,
  ...
  const categoryPivotData = useMemo(() => {
    ...
    if (filledRows.length === 0) {
      return {
        categories: [],
        grandTotal: { qty: 0, fsp: 0, mrp: 0 },
        filteredTotal: { qty: 0, fsp: 0, mrp: 0 },
      };
    }
    ...
    if (pivotGroupBy === "combined") {
      if (pivotCategoryFilter.length > 0) {
        filledRows = filledRows.filter((row: any) =>
          pivotCategoryFilter.includes(row.cms_vertical?.trim()),
        );
      }
      if (pivotBrandFilter.length > 0) {
  ...
  const crossTabFilterField = useMemo(() => {
    const fields: ("category" | "brand" | "p_type")[] = [
      "category",
      "brand",
      "p_type",
    ];
    return (
      fields.find((f) => f !== crossTabRowField && f !== crossTabColField) ||
      "category"
    );
  ...
  const crossTabPivotData = useMemo(() => {
    ...
    if (crossTabFilter.length > 0) {
      ...
      filledRows = filledRows.filter((row: any) =>
        crossTabFilter.includes(row[filterDbField]?.trim()),
      );
    }
  ...
  const handlePivotGroupChange = (
    newGroup: "category" | "brand" | "p_type" | "combined" | "crosstab",
  ) => {
  ...
  const handleCategoryPivotSort = (
    column: "category" | "qty" | "fsp" | "mrp",
  ) => {
  ...
  const exportCategoryPivotToExcel = async () => {
    try {
      ...
      if (pivotGroupBy === "crosstab") {
        ...
        const rowLabel =
          crossTabRowField === "category"
            ? "Category"
            : crossTabRowField === "brand"
              ? "Brand"
              : "P_Type";
        const colLabel =
          crossTabColField === "category"
            ? "Category"
            : crossTabColField === "brand"
              ? "Brand"
              : "P_Type";
        ...
        XLSX.writeFile(
          wb,
          `CrossTab_${rowLabel}_${colLabel}_${timestamp}.xlsx`,
        );
  ...
  const columnDefs = useMemo(() => {
    ...
    const srCol = {
      ...
      valueGetter: (params: any) =>
        params.node ? params.node.rowIndex + 1 : undefined,
      width: 20,
  ...
  const handleMultiCellEditingStopped = useCallback(
    async (event: any) => {
  ...
  useRealtimeSync({
    ...
    onDataSubmitted: useCallback(
      (data: any) => {
        toast.success(
          `${data.submittedBy} submitted ${data.successCount} outbound entries from another device`,
          { duration: 4000, icon: "📡" },
        );
    ...
    onDraftUpdated: useCallback((data: any) => {
      ...
      toast("Outbound draft updated from another device", {
        duration: 3000,
        icon: "📝",
      });
    }, []),
    onDraftCleared: useCallback(() => {
      toast("Outbound draft cleared from another device", {
        duration: 3000,
        icon: "🗑️",
      });
    ...
    onHeaderUpdated: useCallback((data: any) => {
      ...
      if (data.commonVehicle !== undefined)
        setCommonVehicle(data.commonVehicle);
      ...
      setTimeout(() => {
        isHeaderSyncingRef.current = false;
      }, 500);
      toast("Header fields updated from another device", {
        duration: 2000,
        icon: "📡",
      });
    }, []),
  ...
  return (
    <AppLayout>
      ...
      >
          ...
          >
                ...
                (!listData || listData.length === 0) && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        No outbound items match your current filters. Try
                        adjusting your search criteria or reset filters to see
                        all items.
                      </Typography>
                    </Box>
                  </Box>
                            ...
                            if (allColIds.length > 0) {
                              ...
                              setTimeout(() => {
                                try {
                                  ...
                                  total += srCol?.getActualWidth
                                    ? srCol.getActualWidth()
                                    : 80;

                                  const dims = params.api.getSize
                                    ? params.api.getSize()
                                    : null;
            ...
            >
              ...
              <DialogContent>
                <Typography
                  variant="subtitle2"
                  sx={{ fontWeight: 700, color: "#6b21a8", mb: 1 }}
                >
                  EDITABLE FIELDS
                </Typography>
                      ...
                      control={
                        <Checkbox
                          ...
                          onChange={() => {
                            ...
                            const ordered = ALL_LIST_COLUMNS.filter((c) =>
                              next.includes(c),
                            );
              ...
              <DialogActions sx={{ justifyContent: "space-between" }}>
                <Button onClick={() => setListColumnSettingsOpen(false)}>
                  CLOSE
                </Button>
                <Button
                ...
                <Stack spacing={3}>
                  ...
                  >
                    ...
                    >
                      {(exportCustomer || customerFilter) && (
                        <Chip
                          size="small"
                          label={`👤 ${exportCustomer || customerFilter}`}
                          sx={{
                            bgcolor: isDarkMode
                              ? "rgba(59, 130, 246, 0.2)"
                              : "#dbeafe",
                      ...
                      {(exportSource || sourceFilter) && (
                        <Chip
                          size="small"
                          label={`📦 ${exportSource || sourceFilter}`}
                          sx={{
                            bgcolor: isDarkMode
                              ? "rgba(34, 197, 94, 0.2)"
                              : "#dcfce7",
                      ...
                      {(exportBatchId || batchFilter) && (
                        <Chip
                          size="small"
                          label={`🏷️ Batch: ${exportBatchId || batchFilter}`}
                          sx={{
                            bgcolor: isDarkMode
                              ? "rgba(168, 85, 247, 0.2)"
                              : "#f3e8ff",
                      ...
                      {(exportStartDate || startDateFilter) && (
                        <Chip
                          size="small"
                          label={`📅 From: ${exportStartDate || startDateFilter}`}
                          sx={{
                            bgcolor: isDarkMode
                              ? "rgba(99, 102, 241, 0.2)"
                              : "#e0e7ff",
                      ...
                      {(exportEndDate || endDateFilter) && (
                        <Chip
                          size="small"
                          label={`📅 To: ${exportEndDate || endDateFilter}`}
                          sx={{
                            bgcolor: isDarkMode
                              ? "rgba(99, 102, 241, 0.2)"
                              : "#e0e7ff",
                  ...
                  <Autocomplete
                    ...
                    onChange={(event, newValue) =>
                      setExportCustomer(newValue || "")
                    }
                    onInputChange={(event, newInputValue) =>
                      setExportCustomer(newInputValue)
                    }
                    renderInput={(params) => (
                      <TextField
                        {...params}
                  ...
                  >
                    <CardContent sx={{ py: 2, px: 3 }}>
                      ...
                      >
                        This will export filtered outbound/dispatch records to
                        Excel with all selected criteria applied. The file will
                        include dispatch details, product information, and
                        customer data.
                      </Typography>
                    </CardContent>
                  </Card>
              ...
              <CardContent sx={{ p: 2 }}>
                ...
                <Stack spacing={1.5}>
                  ...
                  <TextField
                    ...
                    inputRef={singleDispatchDateInputRef}
                    onClick={() => {
                      try {
                        singleDispatchDateInputRef.current?.showPicker?.();
                      } catch {
                        singleDispatchDateInputRef.current?.focus();
                      }
                    }}
                    sx={{
                      "& .MuiInputBase-root": {
                  ...
                  <CustomerAutocomplete
                    ...
                    onChange={(newValue) =>
                      setSingleForm({ ...singleForm, customer_name: newValue })
                    }
                    customers={customers}
                    warehouseId={activeWarehouse?.id}
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      setSingleForm({
                        ...singleForm,
                        dispatch_remarks: e.target.value,
                      })
                    }
                  />
                    ...
                    <Box>
                      ...
                      <Box sx={{ mt: 0.5 }}>
                        <Chip
                          ...
                          size="small"
                          color={
                            sourceData.source === "PICKING"
                              ? "primary"
                              : sourceData.source === "QC"
                                ? "success"
                                : "warning"
                          }
                          sx={{ fontWeight: 700 }}
            ...
            {duplicateWSN && (
              <Card
                sx={{
                  gridColumn: "1 / -1",
                  borderRadius: 2,
                  background: isDarkMode ? "#78350f" : "#fff3cd",
                  border: `2px solid ${isDarkMode ? "#fbbf24" : "#ffc107"}`,
                }}
              >
                <CardContent>
                  <Typography
                  ...
                  >
                    ⚠️ Duplicate WSN Detected
                  </Typography>
                  <Typography
                    variant="body2"
                    sx={{ color: isDarkMode ? "#fde68a" : "text.secondary" }}
                  >
                    This WSN is already dispatched. Enable "Update Existing
                    Entry" checkbox to modify.
                  </Typography>
                </CardContent>
              </Card>
            )}
            ...
            <BulkUploadCard
              ...
              dropdownFields={[
                ...
                },
                {
                  ...
                  type: "autocomplete",
                  required: true,
                  options: (customers || []).map((c: string) => ({
                    label: c,
                    value: c,
                  })),
                },
          ...
          >
              ...
              >
                      ...
                      >
                        ...
                        >
                          ...
                          <TextField
                            ...
                            value={commonVehicle}
                            onChange={(e) =>
                              setCommonVehicle(e.target.value.toUpperCase())
                            }
                            size="small"
                    ...
                    >
                      ...
                      <Tooltip title="Mobile Scan">
                        <IconButton
                          size="small"
                          onClick={() =>
                            router.push("/mobile-scan?mode=outbound")
                          }
                          sx={{
                            width: 36,
                            height: 36,
                  ...
                  >
                    ...
                    >
                      <TextField
                        ...
                        value={commonDate}
                        onChange={(e) => setCommonDate(e.target.value)}
                        inputRef={multiDispatchDateInputRef}
                        onClick={() => {
                          try {
                            multiDispatchDateInputRef.current?.showPicker?.();
                          } catch {
                            multiDispatchDateInputRef.current?.focus();
                          }
                        }}
                        sx={{
                          width: 150,
                ...
                >
                  ...
                  >
                    ...
                    <IconButton
                      size="small"
                      onClick={() => setOutboundSettingsPanelOpen(false)}
                      sx={{ color: "white" }}
                    >
                      <CloseIcon />
                    </IconButton>
                  ...
                  <Box sx={{ overflow: "auto", flex: 1 }}>
                    ...
                    <Accordion
                      ...
                      onChange={(_, isExpanded) =>
                        setSettingsPanelExpanded(isExpanded ? "columns" : false)
                      }
                      disableGutters
                      sx={{
                        bgcolor: "transparent",
                    ...
                    <Accordion
                      ...
                      onChange={(_, isExpanded) =>
                        setSettingsPanelExpanded(isExpanded ? "cache" : false)
                      }
                      disableGutters
                      sx={{
                        bgcolor: "transparent",
                      ...
                      >
                        ...
                        >
                          ...
                          <Box>
                            ...
                            >
                              {cacheEnabled
                                ? `${cacheStats?.totalRecords?.toLocaleString() || 0} items cached`
                                : "Cache disabled"}
                            </Typography>
                          </Box>
                        </Box>
                        ...
                        <Stack spacing={2}>
                            ...
                            sx={{
                              ...
                              bgcolor: ctrlOProductLinkEnabled
                                ...
                                : isDarkMode
                                  ? "#0f172a"
                                  : "#f8fafc",
                              border: ctrlOProductLinkEnabled
                                ? "1px solid #a855f7"
                                : isDarkMode
                                  ? "1px solid #334155"
                                  : "1px solid #e2e8f0",
                          ...
                          >
                            <Box>
                              ...
                              >
                                Opens FKT link for last scanned WSN
                              </Typography>
                            </Box>
                            <Switch
                    ...
                    >
                      <Button
                        ...
                        onClick={() => {
                          setCategoryPivotOpen(true);
                          setOutboundSettingsPanelOpen(false);
                        }}
            ...
            >
              ...
              <DialogContent>
                <Typography variant="body2" color="text.secondary">
                  Export {multiRows.filter((r: any) => r.wsn?.trim()).length}{" "}
                  rows with WSN data to Excel file?
                </Typography>
                <Typography
                  variant="caption"
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <IconButton
                    size="small"
                    onClick={() =>
                      setPivotDialogFullscreen(!pivotDialogFullscreen)
                    }
                    sx={{
                      color: "white",
                    ...
                    title={
                      pivotDialogFullscreen ? "Exit Fullscreen" : "Fullscreen"
                    }
                  >
                    {pivotDialogFullscreen ? (
                      <FullscreenExitIcon />
              ...
              <DialogContent sx={{ p: 0 }}>
                ...
                >
                  ...
                  <Box sx={{ display: "flex", gap: 0.5 }}>
                    <Button
                      size="small"
                      variant={
                        pivotGroupBy === "category" ? "contained" : "outlined"
                      }
                    ...
                    <Button
                      size="small"
                      variant={
                        pivotGroupBy === "brand" ? "contained" : "outlined"
                      }
                    ...
                    <Button
                      size="small"
                      variant={
                        pivotGroupBy === "p_type" ? "contained" : "outlined"
                      }
                    ...
                    <Button
                      size="small"
                      variant={
                        pivotGroupBy === "combined" ? "contained" : "outlined"
                      }
                    ...
                    <Button
                      size="small"
                      variant={
                        pivotGroupBy === "crosstab" ? "contained" : "outlined"
                      }
                ...
                {pivotGroupBy === "combined" && (
                  ...
                  >
                    ...
                    <Autocomplete
                      ...
                      onChange={(_, newValue) =>
                        setPivotCategoryFilter(newValue)
                      }
                      renderInput={(params) => (
                        <TextField
                          {...params}
                          label="Category"
                          placeholder={
                            pivotCategoryFilter.length === 0 ? "All" : ""
                          }
                          sx={{
                            minWidth: 180,
                            "& .MuiInputBase-root": {
                    ...
                    />
                    {(pivotCategoryFilter.length > 0 ||
                      pivotBrandFilter.length > 0 ||
                      pivotPTypeFilter.length > 0) && (
                      <Button
                        size="small"
                        onClick={() => {
                          setPivotCategoryFilter([]);
                ...
                {pivotGroupBy === "crosstab" ? (
                  crossTabPivotData.grandTotal === 0 ? (
                    ...
                    <TableContainer
                      sx={{
                        maxHeight: pivotDialogFullscreen
                          ? "calc(92vh - 220px)"
                          : 500,
                      }}
                    >
                              ...
                              {crossTabPivotData.colHeaders.map((col) => (
                                <TableCell
                                  ...
                                  sx={{
                                    fontSize: "0.8rem",
                                    color:
                                      crossTabPivotData.data[row][col] > 0
                                        ? isDarkMode
                                          ? "#34d399"
                                          : "#059669"
                                        : isDarkMode
                        ...
                        <TableRow>
                          ...
                          {pivotGroupBy === "combined" ? (
                            <>
                              <TableCell
                                onClick={() =>
                                  handleCategoryPivotSort("category")
                                }
                                sx={{
                                  fontWeight: 700,
                                  cursor: "pointer",
                              ...
                              >
                                Category{" "}
                                {categoryPivotSortBy === "category" &&
                                  (categoryPivotSortDir === "asc" ? "↑" : "↓")}
                              </TableCell>
                              <TableCell
                                sx={{
                                  fontWeight: 700,
              ...
              <AgGridReact
                ...
                onColumnResized={(params: any) => {
                  if (
                    ...
                    multiColumnWidthsRef.current = {
                      ...multiColumnWidthsRef.current,
                      [colId]: newWidth,
                    };
                    localStorage.setItem(
                      "outboundMultiEntryColumnWidths",
                      JSON.stringify(multiColumnWidthsRef.current),
                    );
                  }
                }}
                onGridReady={(params: any) => {
                      ...
                      const colsToAutoSize = allCols
                        ...
                        .filter(
                          (id: string) =>
                            id !== "__sr" &&
                            id !== "_filler" &&
                            !multiColumnWidthsRef.current[id],
                        );
            ...
            >
              ...
              <Button
                ...
                disabled={
                  multiLoading || !activeWarehouse || duplicateWSNs.size > 0
                }
                startIcon={
                  multiLoading ? (
              ...
              {selectionStats && selectionStats.numericCount > 0 && (
                <Chip
                  ...
                  label={
                    ...
                    >
                      <span
                        style={{ color: isDarkMode ? "#22d3ee" : "#2563eb" }}
                      >
        ...
        <Dialog
          open={preSubmitExportDialogOpen}
          onClose={() => setPreSubmitExportDialogOpen(false)}
          maxWidth="xs"
          fullWidth
        ...
        >
          ...
          <DialogContent>
            <Typography variant="body2" color="text.secondary">
              ...
              <strong>
                {multiRows.filter((r) => r.wsn?.trim()).length} rows
              </strong>{" "}
              to Excel before submitting?
            </Typography>
          </DialogContent>
          <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
            <Button
              onClick={() => setPreSubmitExportDialogOpen(false)}
              size="small"
        ...
        <Dialog
          open={multiExportDialogOpen}
          onClose={() => setMultiExportDialogOpen(false)}
          maxWidth="sm"
          fullWidth
          fullScreen={isMobile}
          container={isFullscreen ? multiEntryContainerRef.current : undefined}
        >
          ...
          <DialogContent>
            ...
            >
              ...
              Filename:{" "}
              <strong>
                {(selectedCustomer || commonCustomer || "Customer")
                  .replace(/[^a-zA-Z0-9\s-]/g, "")
                  .trim()
                  .replace(/\s+/g, "_")}
                _Dispatch_
                {(() => {
                  ...
                  const d = commonDate ? new Date(commonDate) : new Date();
            ...
            <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
              <Button
                ...
                onClick={() =>
                  setExportSelectedColumns(
                    EXPORT_COLUMNS_CONFIG.map((c) => c.key),
                  )
                }
              ...
              <Button
                ...
                variant="text"
                onClick={() =>
                  setExportSelectedColumns([...DEFAULT_EXPORT_COLUMNS])
                }
                sx={{
                  fontSize: "0.7rem",
                  textTransform: "none",
            ...
            >
              Selected: {exportSelectedColumns.length} /{" "}
              {EXPORT_COLUMNS_CONFIG.length} columns •{" "}
              {multiRows.filter((r) => r.wsn?.trim()).length} rows to export
            </Typography>
          </DialogContent>
          <DialogActions sx={{ px: 3, pb: 2 }}>
            <Button onClick={() => setMultiExportDialogOpen(false)}>
              Cancel
            </Button>
            <Button
              variant="contained"
        ...
        {currentTabCode === "batches" && (
          <BatchManagementTab
            ...
            onDelete={
              canSeeButton("batches:delete") ? handleDeleteBatch : undefined
            }
              ...
              <AccordionDetails sx={{ px: 2, pb: 2, pt: 0 }}>
                <Stack spacing={1.5}>
                  ...
                  >
                    ...
                    {batches.map((b: any) => (
                      <MenuItem key={b.batch_id} value={b.batch_id}>
                        {b.batch_id} {b.count ? `(${b.count})` : null}
                      </MenuItem>
                    ))}
                  </TextField>
                </Stack>
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    ...
                    <Box>
                      ...
                      >
                        {listColumns.length} of {ALL_LIST_COLUMNS.length}{" "}
                        visible
                      </Typography>
                    </Box>
                  </Box>
