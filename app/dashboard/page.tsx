...
const getCachedDashboardData = (): InventoryItem[] => {
  ...
  if (
    ...
    window.__DASHBOARD_CACHE__?.data?.length
  ) {
    // Only use cache if warehouse matches and not stale (2 min TTL)
    if (
      window.__DASHBOARD_CACHE__.warehouseId === currentWarehouseId &&
  ...
  try {
    if (typeof window !== "undefined") {
      ...
      const savedWarehouseId = sessionStorage.getItem(
        "dashboard_last_data_warehouseId",
      );
      if (
        saved &&
        savedWarehouseId &&
...
export default function DashboardPage() {
  ...
  const didInitialMountRef = useRef(
    (() => {
      try {
        return (
          sessionStorage.getItem("dashboard_mounted_this_session") === "true"
        );
      } catch {
        return false;
      }
  ...
  const [loading, setLoading] = useState(
    () => getCachedDashboardData().length === 0,
  );
  ...
  // ====== DATE FILTER REFS ======
  const exportFromDateRef = useRef<HTMLInputElement | null>(null);
  const exportToDateRef = useRef<HTMLInputElement | null>(null);
  ...
  const [initialLoad, setInitialLoad] = useState<boolean>(() => {
    try {
      return !localStorage.getItem("dashboard_ever_loaded");
    } catch {
      return true;
    }
  });
  ...
  const previousDataRef = useRef<InventoryItem[] | null>(
    getCachedDashboardData().length > 0 ? getCachedDashboardData() : null,
  );
  ...
  const pageCacheRef = useRef<
    Map<string, { data: InventoryItem[]; total: number; timestamp: number }>
  >(
    (() => {
      try {
        ...
        if (saved) {
          ...
          return new Map(Object.entries(parsed)) as Map<
            string,
            { data: InventoryItem[]; total: number; timestamp: number }
          >;
        }
      } catch {
        /* ignore */
  ...
  const [enableSorting, setEnableSorting] = useState<boolean>(() => {
    try {
      return localStorage.getItem("dashboard_enableSorting") === "true";
    } catch {
      return false;
    }
  });
  const [enableColumnFilters, setEnableColumnFilters] = useState<boolean>(
    () => {
      try {
        return localStorage.getItem("dashboard_enableColumnFilters") === "true";
      } catch {
        return false;
      }
    },
  ...
  const [enableColumnResize, setEnableColumnResize] = useState<boolean>(() => {
    try {
      return localStorage.getItem("dashboard_enableColumnResize") !== "false";
    } catch {
      return true;
    }
  });
  const [enableCellEditing, setEnableCellEditing] = useState<boolean>(() => {
    try {
      return localStorage.getItem("dashboard_enableCellEditing") !== "false";
    } catch {
      return true;
    }
  });
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem(
        "dashboard_enableColumnFilters",
        String(enableColumnFilters),
      );
      localStorage.setItem(
        "dashboard_enableColumnResize",
        String(enableColumnResize),
      );
      localStorage.setItem(
        "dashboard_enableCellEditing",
  ...
  const getColumnSizing = useCallback(
    (col: string) => {
      ...
      if (isMobile && sizing.mobile)
        return {
          ...sizing.mobile,
          ...(sizing.mobile.width ? { suppressSizeToFit: true } : {}),
        };
  ...
  useEffect(() => {
    ...
    const srCol = {
      ...
      field: "__sr",
      valueGetter: (params: any) =>
        params.node ? (page - 1) * limit + params.node.rowIndex + 1 : undefined,
      width: 20,
      maxWidth: 50,
      suppressMovable: true,
      sortable: false,
            ...
            cellRenderer: (p: any) => {
              ...
              if (!raw)
                return (
                  <Chip
                    label="-"
                    size="small"
                    sx={{ bgcolor: "#999", color: "white", fontSize: "0.7rem" }}
                  />
                );
              ...
              return (
                <Chip
                  ...
                  size="small"
                  sx={{
                    bgcolor: getStatusColor(normalized),
                    color: "white",
                    fontSize: "0.7rem",
                  }}
                />
  ...
  const handleWSNLookup = useCallback(
    async (wsn: string) => {
      ...
      try {
        const res = await inboundAPI.getMasterDataByWSN(
          trimmed,
          activeWarehouse.id,
        );
        if (res.data) {
  ...
  const loadInventoryData = useCallback(
    async (forceRefresh = false, showOverlay = false) => {
      ...
      try {
        ...
        if (loadId === currentLoadIdRef.current) {
          ...
          if (initialLoad) {
            ...
            try {
              localStorage.setItem("dashboard_ever_loaded", "true");
            } catch {}
          }
  ...
  const loadFilterOptions = useCallback(async () => {
    ...
    try {
      ...
      const brandsData = Array.isArray(bResp?.data)
        ? bResp.data
        : bResp?.data?.data || [];
      const categoriesData = Array.isArray(cResp?.data)
        ? cResp.data
        : cResp?.data?.data || [];
  ...
  const loadExistingStageWSNs = useCallback(async () => {
    try {
      ...
      const pList = Array.isArray(pResp?.data)
        ? pResp.data
        : pResp?.data?.data || [];
      const oList = Array.isArray(oResp?.data)
        ? oResp.data
        : oResp?.data?.data || [];
  ...
  useEffect(() => {
    if (activeWarehouse) {
      ...
      inventoryLoadDebounceRef.current = setTimeout(() => {
        try {
          sessionStorage.setItem("dashboard_mounted_this_session", "true");
        } catch {}
    ...
    const autoSize = () => {
      try {
        ...
        const allColIds = api.getColumns
          ...
          : (columnApiRef.current?.getAllColumns?.() || []).map((c: any) =>
              c.getColId(),
            );
        ...
        let total = 0;
        const getCol =
          api.getColumn?.bind(api) ||
          columnApiRef.current?.getColumn?.bind(columnApiRef.current);
        if (getCol) {
  ...
  const memoizedFilteredBrands = useMemo(() => {
    ...
    return Array.from(
      new Set(
        filteredByCategory.map((item: any) => item.brand).filter(Boolean),
      ),
    ).sort() as string[];
  ...
  const handleExportWithFilters = async () => {
    ...
    try {
      ...
      if (activeWarehouse?.id)
        params.append("warehouseId", String(activeWarehouse.id));
      ...
      if (exportFilters.availableOnly === "available")
        ...
        >
          ...
          >
            {[
              ...
              {
                label: "Processed",
                value: (metrics.qcPassed || 0) + (metrics.qcDone || 0),
                color: "#06b6d4",
                icon: <DoneAllRounded />,
              ...
              {
                label: "Available",
                value: inventorySummary.available_stock,
                color: "#10b981",
                ...
                sx={{
                  ...
                  "&:hover": {
                    ...
                    boxShadow: {
                      xs: isDarkMode
                        ? `0 4px 12px ${m.color}15, inset 0 1px 1px rgba(255,255,255,0.05)`
                        : `0 2px 10px ${m.color}20, inset 0 1px 1px rgba(255,255,255,0.85)`,
                      md: isDarkMode
                        ? `0 12px 32px ${m.color}25, inset 0 1px 1px rgba(255,255,255,0.08)`
                        : `0 12px 32px ${m.color}28, inset 0 1px 1px rgba(255,255,255,0.8)`,
                    },
                  },
                }}
              >
                ...
                >
                  ...
                  <Box
                    sx={{
                      ...
                      width: "100%",
                      maskImage: {
                        xs: "linear-gradient(to right, black 70%, transparent 100%)",
                        sm: "none",
                      },
                      WebkitMaskImage: {
                        xs: "linear-gradient(to right, black 70%, transparent 100%)",
            ...
            >
              ...
              >
                ...
                <Tooltip title="Open Pivot Analysis - Excel-style pivot table">
                  ...
                  >
                    <PivotIcon
                      sx={{ fontSize: "1.1rem", mr: { xs: 0, md: 0.5 } }}
                    />
                    <Box
                      component="span"
                ...
                >
                  ...
                  >
                    ...
                    {((loading &&
                      ...
                      activeWarehouse) ||
                      (gridRowData.length > 0 && !gridDataRendered)) && (
                      <Box
                        sx={{
                          position: "absolute",
                          top: 35,
        ...
        <Box sx={{ overflow: "auto", flex: 1 }}>
          ...
          <Accordion
            ...
            onChange={(_, isExpanded) =>
              setSettingsPanelExpanded(isExpanded ? "filters" : false)
            }
            disableGutters
            sx={{
              bgcolor: "transparent",
            ...
            <Box>
              ...
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  inputRef={exportFromDateRef}
                  onMouseDown={(e) => {
                    e.preventDefault();
                  }}
                  onClick={() => {
                    try {
                      exportFromDateRef.current?.showPicker?.();
                    } catch {
                      exportFromDateRef.current?.focus();
                    }
                  }}
                  fullWidth
                  sx={{
                ...
                <TextField
                  ...
                  inputRef={exportToDateRef}
                  onMouseDown={(e) => {
                    e.preventDefault();
                  }}
                  onClick={() => {
                    try {
                      exportToDateRef.current?.showPicker?.();
                    } catch {
                      exportToDateRef.current?.focus();
                    }
                  }}
                  fullWidth
                  sx={{
      ...
      >
        ...
        >
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            >
              <SearchIcon
                sx={{ fontSize: 18, color: isDarkMode ? "#60a5fa" : "#fff" }}
              />
            </Box>
            <Typography
              sx={{
          ...
          <Box sx={{ px: 2, py: 1.5 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              <TextField
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") handleWSNLookup(lookupWSN);
                }}
                sx={{
                  "& .MuiOutlinedInput-root": {
                    height: 44,
            ...
            <Box sx={{ px: 2, pb: 2 }}>
              ...
              {lookupResult.fkt_link && (
                <Button
                  ...
                  startIcon={
                    <OpenInNewIcon sx={{ fontSize: "16px !important" }} />
                  }
                  sx={{
                    textTransform: "none",
                    fontWeight: 700,
                    ...
                    bgcolor: isDarkMode ? "rgba(59,130,246,0.06)" : "#f0f7ff",
                    "&:hover": {
                      bgcolor: isDarkMode ? "rgba(59,130,246,0.12)" : "#dbeafe",
                      borderColor: "#3b82f6",
                    },
                  }}
                >
