...
export default function NLInboundPage() {
  ...
  const { filterTabs, canSeeTab, canSeeButton, isAdmin } =
    usePagePermissions("nl_inbound");
  ...
  const [autoPrintEnabled, setAutoPrintEnabled] = useState(() => {
    if (typeof window !== "undefined") {
      const s = localStorage.getItem("nl_inbound_autoPrint");
      return s !== "false";
    }
    return true;
  });
  ...
  const [listSettingsExpanded, setListSettingsExpanded] = useState<
    string | false
  >("filters");
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const s = localStorage.getItem("nl_inbound_visible_cols");
        return s ? JSON.parse(s) : ALL_GRID_COLUMNS;
      } catch {
  ...
  useEffect(() => {
    const checkAgent = async () => {
      setAgentReady(await isAgentRunning());
    };
    checkAgent();
  ...
  useEffect(() => {
    localStorage.setItem("nl_inbound_autoPrint", String(autoPrintEnabled));
  }, [autoPrintEnabled]);
  ...
  const generateEmptyRows = useCallback((count: number) => {
    return Array.from({ length: count }, () => {
      ...
      return {
        ...
        lot_type: "",
        region: "",
        declared_qty: null as number | null,
        actual_qty: null as number | null,
        short_qty: null as number | null,
        route: "direct",
        remarks: "",
      };
    });
      ...
      if (saved) {
        ...
        if (draft.rows?.length > 0) {
          ...
          const padding = generateEmptyRows(
            Math.max(500 - restoredRows.length, 100),
          );
  ...
  // ======================== PRINT BOX STICKER ========================
  const printBoxSticker = useCallback(
    async (rowData: any) => {
      if (!rowData?.box_id?.trim()) {
        toast.error("No Box ID to print");
        return;
      }
      if (!agentReady) {
      ...
      try {
        ...
        if (success)
          toast.success(`Sticker printed: ${rowData.box_id}`, {
            duration: 1500,
          });
  ...
  // ======================== VRP LOOKUP ========================
  const lookupVRP = useCallback(
    async (vrpId: string, rowIndex: number) => {
      ...
      try {
        const res = await nlFktOsAPI.getByVrpId(
          vrpId.trim().toUpperCase(),
          activeWarehouse?.id,
        );
        if (res.data?.data) {
          ...
          const completeRow = {
            ...currentRow,
            vrp_id: vrpId.trim().toUpperCase(),
            category: osData.category || "",
            lot_type: osData.lot_type || "",
            region: osData.region || "",
            declared_qty: osData.declared_qty || 0,
          ...
          setRows((prev) => {
            const u = [...prev];
            u[rowIndex] = { ...completeRow };
            return u;
          });
          setTimeout(() => {
            const api = gridRef.current?.api;
            if (api) {
              const n = api.getRowNode(completeRow._rowId);
              if (n) api.refreshCells({ rowNodes: [n], force: true });
            }
          }, 50);
          ...
          setTimeout(() => {
            const api = gridRef.current?.api;
            if (api) {
              api.setFocusedCell(rowIndex, "actual_qty");
          ...
          setRows((prev) => {
            ...
            if (u[rowIndex])
              u[rowIndex] = {
                ...u[rowIndex],
                _error: "VRP not found in OS data",
              };
            return u;
          });
        }
  ...
  const handleCellValueChanged = useCallback(
    (event: any) => {
      ...
      setRows((prev) => {
        const u = [...prev];
        u[rowIndex] = { ...u[rowIndex], [field]: processedValue };
        if (field === "actual_qty" && u[rowIndex].declared_qty != null) {
          u[rowIndex].short_qty =
            (u[rowIndex].declared_qty || 0) - (Number(processedValue) || 0);
        }
        return u;
      });

      if (field === "vrp_id" && processedValue?.trim())
        lookupVRP(processedValue, rowIndex);
    ...
    (params: any) => {
      ...
      if (editing && event.key === "Enter") {
        ...
        if (field === "box_id") {
          setTimeout(() => {
            ...
            setTimeout(
              () => params.api.startEditingCell({ rowIndex, colKey: "vrp_id" }),
              50,
            );
          }, 20);
        }
        // vrp_id -> move to actual_qty immediately, then trigger lookup in background
          ...
          setTimeout(
            () =>
              params.api.startEditingCell({ rowIndex, colKey: "actual_qty" }),
            50,
          );
      ...
      if (editing && event.key === "Tab") {
        ...
        if (
          ...
          currentIndex + direction < WORKFLOW_FIELDS.length
        ) {
      ...
      if (event.ctrlKey && event.key.toLowerCase() === "p") {
        ...
        if (lastScannedRowRef.current)
          printBoxSticker(lastScannedRowRef.current);
        return true;
      }
      return false;
    },
  ...
  const handleSubmit = async () => {
    ...
    if (filledRows.length === 0) {
      toast.error("No boxes to submit");
      return;
    }

    const errorRows = filledRows.filter((r) => !r._vrpFound);
    if (errorRows.length > 0) {
      toast.error(`${errorRows.length} row(s) have unresolved VRP IDs`);
      return;
    }
    ...
    try {
      ...
      if (data.error_count > 0) {
        toast.success(
          `Submitted ${data.total_boxes} boxes (${data.error_count} errors). Batch: ${data.session_id}`,
        );
      } else {
        toast.success(
          `Submitted ${data.total_boxes} boxes. Batch: ${data.session_id}`,
        );
      }
  ...
  const loadInboundList = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load inbound list");
    } finally {
  ...
  }, [currentTabCode, loadInboundList]);
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !activeWarehouse?.id) {
      toast.error("File and active warehouse required");
      return;
    }
    try {
      ...
      const entries = jsonRows
        .map((row) => ({
          box_id: String(row["BOX ID"] || row["Box ID"] || row["box_id"] || "")
            .trim()
            .toUpperCase(),
          vrp_id: String(row["VRP ID"] || row["VRP_ID"] || row["vrp_id"] || "")
            .trim()
            .toUpperCase(),
          actual_qty:
            parseInt(
              row["Actual Qty"] || row["actual_qty"] || row["QTY"] || "0",
              10,
            ) || undefined,
          route:
            String(row["Route"] || row["route"] || "direct").toLowerCase() ===
            "qc"
              ? "qc"
              : "direct",
      ...
      const d = res.data;
      toast.success(
        `Bulk upload: ${d.total_boxes} saved${d.error_count ? `, ${d.error_count} errors` : ""}. Batch: ${d.session_id}`,
      );
    } catch (error: any) {
      toast.error(
        "Bulk upload failed: " +
  ...
  const exportListToExcel = async () => {
    ...
    const ed = listData.map((r) => ({
      ...
      "Declared Qty": r.declared_qty,
      "Actual Qty": r.actual_qty,
      "Short Qty": (r.declared_qty || 0) - (r.actual_qty || 0),
      Route: r.route,
      Remarks: r.remarks || "",
      "Session ID": r.session_id,
      "Inbound Date": formatDate(r.inbound_date),
      "Vehicle No": r.vehicle_no || "",
      "Received By": r.received_by_name || "",
      "Received At": formatDate(r.received_at),
    }));
    const ws = XLSX.utils.json_to_sheet(ed);
    const wb = XLSX.utils.book_new();
    ...
    XLSX.writeFile(
      wb,
      `NL_Inbound_List_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  ...
  const columnDefs = useMemo(() => {
    ...
    visibleColumns.forEach((col: string) => {
      ...
      const colDef: any = {
        ...
        minWidth: ["box_id", "vrp_id"].includes(col)
          ? 130
          : col === "remarks"
            ? 140
            : 100,
      ...
      if (col === "route") {
        ...
        colDef.cellEditorParams = { values: ["direct", "qc"] };
      }
      if (["declared_qty", "actual_qty", "short_qty"].includes(col))
      ...
      if (isAutoField && col !== "short_qty") {
        colDef.cellStyle = { color: isDarkMode ? "#94a3b8" : "#64748b" };
        colDef.editable = false;
      }
      if (col === "box_id") {
        colDef.cellStyle = (p: any) => {
    ...
    cols.push({
      ...
      editable: false,
      cellRenderer: (params: any) => {
        if (params.data._vrpFound)
          return <CheckCircleIcon sx={{ color: "#22c55e", fontSize: 18 }} />;
        if (params.data._error)
          return (
            <Tooltip title={params.data._error}>
              <span style={{ color: "#ef4444", cursor: "help" }}>⚠</span>
            </Tooltip>
          );
        return null;
      },
  ...
  const listColumnDefs = useMemo(
    () => [
      ...
      },
      {
        ...
        type: "numericColumn",
        valueGetter: (p: any) =>
          (p.data?.declared_qty || 0) - (p.data?.actual_qty || 0),
        cellStyle: (p: any) =>
          p.value > 0 ? { color: "#ef4444", fontWeight: 700 } : null,
      },
  ...
  const filledCount = rows.filter(
    (r) => r.box_id?.trim() && r._vrpFound,
  ).length;
  const totalQty = rows
    .filter((r) => r._vrpFound)
    .reduce((sum, r) => sum + (r.actual_qty || 0), 0);
  ...
  return (
    <AppLayout>
      ...
      >
        <StandardPageHeader
          title="NL Inbound"
          subtitle="Multi-entry box scanning"
          icon="📥"
            ...
            <Paper sx={{ p: 1.5, mb: 1, borderRadius: 2 }}>
              ...
              >
                <Stack direction="row" spacing={1.5} alignItems="center">
                  ...
                  {filledCount > 0 && (
                    <Chip
                      label={`${filledCount} boxes — ${totalQty} qty`}
                      size="small"
                      color="primary"
                      variant="outlined"
                      sx={{ fontWeight: 600 }}
                ...
                <Stack direction="row" spacing={0.5} alignItems="center">
                  {filledCount > 0 && (
                    <>
                      ...
                      <Tooltip title="Clear Draft & Grid">
                        <IconButton
                          aria-label="Action"
                          size="small"
                          onClick={clearDraft}
                  ...
                  >
                    <Chip
                      label={agentReady ? "🖨️ Ready" : "🖨️ Off"}
                      size="small"
                      color={agentReady ? "success" : "default"}
                      variant="outlined"
                      sx={{ fontSize: "0.7rem" }}
                    />
                  </Tooltip>
            ...
            >
              <Stack direction="row" spacing={2} alignItems="center">
                <Typography
                  ...
                  sx={{
                    fontWeight: 600,
                    color: filledCount > 0 ? "text.primary" : "text.secondary",
                  }}
                >
                  {filledCount > 0
                    ? `${filledCount} box${filledCount !== 1 ? "es" : ""} ready — ${totalQty} total qty`
                    : "No data entered yet"}
                </Typography>
              </Stack>
          ...
          >
            ...
            >
              <TextField
                fullWidth
                size="small"
                placeholder="Search box ID, VRP, session, vehicle..."
                value={listSearch}
        ...
        {currentTabCode === "bulk" && (
          ...
          >
            ...
            >
              ...
              >
                Upload an Excel file with multiple boxes. All rows will be
              ...
              <Box sx={{ mb: 3, display: "flex", flexWrap: "wrap", gap: 0.75 }}>
                {["BOX ID *", "VRP ID *", "Actual Qty", "Route", "Remarks"].map(
                  (col) => (
                    <Chip
                      key={col}
                      label={col}
              ...
              <Alert severity="info" sx={{ mt: 3 }}>
                All rows will be submitted as a single batch. VRP IDs must exist
                in OS Data.
              </Alert>
            </Paper>
          </Box>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Submit <strong>{filledCount} boxes</strong> with total qty{" "}
            <strong>{totalQty}</strong>
            {vehicleNo ? ` for vehicle ${vehicleNo}` : ""}?
          </Typography>
          <Typography variant="body2" sx={{ mt: 1, color: "text.secondary" }}>
            A Batch ID will be auto-generated. This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <Box sx={{ overflow: "auto", flex: 1, p: 1 }}>
            ...
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography sx={{ fontWeight: 600 }}>
                  Columns ({visibleColumns.length}/{ALL_GRID_COLUMNS.length})
                </Typography>
              </Stack>
            </AccordionSummary>
            <AccordionDetails>
          ...
          <Accordion
            expanded={settingsExpanded === "print"}
            onChange={(_, exp) => setSettingsExpanded(exp ? "print" : false)}
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography sx={{ fontWeight: 600 }}>Printing</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <FormControlLabel
                control={
                ...
                label="Auto Print on VRP Lookup"
                sx={{ display: "block" }}
              />
              <Typography
                variant="caption"
              ...
              >
                Ctrl+P — Reprint last scanned box sticker
              </Typography>
              <Chip
                label={agentReady ? "Agent Connected" : "Agent Not Running"}
                size="small"
                color={agentReady ? "success" : "error"}
                variant="outlined"
                sx={{ mt: 1 }}
              />
            </AccordionDetails>
      ...
      >
        ...
        <Box sx={{ overflow: "auto", flex: 1, p: 2 }}>
          <Accordion
            expanded={listSettingsExpanded === "filters"}
            onChange={(_, exp) =>
              setListSettingsExpanded(exp ? "filters" : false)
            }
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Stack direction="row" spacing={1} alignItems="center">
