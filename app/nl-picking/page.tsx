...
import {
    ...
    Send as SendIcon,
    Delete as DeleteIcon, Cancel as CancelIcon,
...
export default function NLPickingPage() {
    ...
    const { filterTabs, canSeeTab, isAdmin } = usePagePermissions('nl_picking');
    ...
    const [plItems, setPlItems] = useState<{ vrp_id: string; required_qty: number }[]>([{ vrp_id: '', required_qty: 1 }]);
    ...
    const [pickTotals, setPickTotals] = useState({ total_boxes: 0, total_qty: 0 });
    ...
    const [discardPickingDialogOpen, setDiscardPickingDialogOpen] = useState(false);
    const [cancelPicklistTargetId, setCancelPicklistTargetId] = useState<string | null>(null);
    // Pick guide
    ...
    const generatePickEmptyRows = useCallback((count: number) => {
        return Array.from({ length: count }, () => {
            ...
            return {
                ...
                id: '', box_id: '', vrp_id: '', rack_no: '', category: '', lot_type: '', qty: null, picked_at: '',
            };
        });
    }, []);

    useEffect(() => { pickRowsRef.current = pickRows; }, [pickRows]);
    ...
    const handlePlItemChange = (idx: number, field: 'vrp_id' | 'required_qty', value: any) => {
        setPlItems(prev => prev.map((item, i) => i === idx ? { ...item, [field]: value } : item));
    };
    ...
    const parsePastedItems = (text: string): { vrp_id: string; required_qty: number }[] => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                for (const row of rows) {
                    const vrpKey = Object.keys(row).find(k => /vrp|item|sku|product/i.test(k)) || Object.keys(row)[0];
                    const qtyKey = Object.keys(row).find(k => /qty|quantity|count|pcs/i.test(k)) || Object.keys(row)[1];
                    const vrp = String(row[vrpKey] || '').replace(/[^a-zA-Z0-9\-_]/g, '').trim();
                    if (!vrp) continue;
    ...
    const handleCreatePicklist = async () => {
        if (!activeWarehouse?.id) return;
        const validItems = plItems.filter(i => i.vrp_id.trim() && i.required_qty > 0);
    ...
    const handleCancelPicklist = async (plId: string) => {
        try {
            ...
            loadPicklists();
            if (expandedPl?.id === plId) { setExpandedPl(null); setExpandedPlItems([]); }
        } catch (error: any) {
            ...
            if (res.data?.exists && res.data?.session) {
                ...
                const scannedRows = entries.map((e: any) => {
                    ...
                    return {
                        ...
                        rack_no: e.rack_no, category: e.category, lot_type: e.lot_type,
                        qty: e.qty, picked_at: e.picked_at,
                    };
                });
    ...
    const loadPickGuide = async (plId: string) => {
        ...
        finally { setPickGuideLoading(false); }
    };
    ...
    const handleStartPicking = async () => {
        ...
        try {
            ...
            setTimeout(() => {
                const api = pickGridRef.current?.api;
                if (api) { api.setFocusedCell(0, 'box_id'); api.startEditingCell({ rowIndex: 0, colKey: 'box_id' }); }
            }, 200);
        } catch (error: any) {
    ...
    const scanBoxForRow = useCallback(async (boxId: string, rowIndex: number) => {
        ...
        try {
            ...
            toast.success(`Box ${entry.box_id} picked from ${entry.rack_no || '?'}`);
    ...
    const handlePickCellChanged = useCallback((event: any) => {
    ...
    const suppressPickKeyboardEvent = useCallback((params: any) => {
    ...
    const loadPickingList = useCallback(async () => {
        ...
        try {
            ...
            const res = await nlPickingAPI.listSessions({ warehouse_id: activeWarehouse.id });
            setPickingListData((res.data?.data || []).filter((s: any) => s.status === 'submitted'));
        } catch {
    ...
    const pickEntryColDefs = useMemo(() => [
        {
            headerName: '', width: 45, sortable: false, filter: false, suppressKeyboardEvent: suppressPickKeyboardEvent,
            cellRenderer: (p: any) => {
        ...
        { field: 'vrp_id', headerName: 'VRP ID', width: 130, suppressKeyboardEvent: suppressPickKeyboardEvent },
        { field: 'rack_no', headerName: 'Rack', width: 100, suppressKeyboardEvent: suppressPickKeyboardEvent },
        { field: 'category', headerName: 'Category', width: 120, suppressKeyboardEvent: suppressPickKeyboardEvent },
        { field: 'lot_type', headerName: 'Lot', width: 70, suppressKeyboardEvent: suppressPickKeyboardEvent },
        { field: 'qty', headerName: 'Qty', width: 70, type: 'numericColumn', suppressKeyboardEvent: suppressPickKeyboardEvent },
        {
            field: 'picked_at', headerName: 'Time', width: 110, suppressKeyboardEvent: suppressPickKeyboardEvent,
        ...
        },
        {
            ...
            cellRenderer: (p: any) => p.data._scanned ? (
                <IconButton aria-label="Action" size="small" color="error" onClick={() => handleDeletePickEntry(p.data.id, p.node.rowIndex)} title="Remove">
    ...
    const pickingListColDefs = useMemo(() => [
        ...
        { field: 'picklist_code', headerName: 'Picklist', flex: 1, minWidth: 180 },
        ...
        {
            field: 'submitted_at', headerName: 'Submitted', width: 130,
    ...
    const pickGuideSummary = useMemo(() => {
        ...
        return pickGuide.items.map((item: any) => {
            const boxes = pickGuide.available_boxes.filter(
                (b: any) => (b.vrp_id || '').toUpperCase() === (item.vrp_id || '').toUpperCase()
            );
            // Group by rack
    ...
    return (
        <AppLayout>
            ...
            <Box sx={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: isDarkMode ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' : 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)' }}>
                <StandardPageHeader title="NL Picking" subtitle="Picklist & picking management" icon="📋" warehouseName={activeWarehouse?.name} />
                <StandardTabs value={tabValue} onChange={(_, v) => setTabValue(v)} tabs={visibleTabs} color="#059669" />

                {/* ===== PICKLIST TAB ===== */}
                {currentTabCode === 'picklist' && (
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'auto', p: { xs: 1, sm: 1.5 } }}>
                        ...
                        <Paper sx={{ p: 2, mb: 1.5, borderRadius: 2 }}>
                            ...
                            <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} sx={{ mb: 1.5 }}>
                                <TextField label="Customer (optional)" value={plCustomerId} onChange={(e) => setPlCustomerId(e.target.value ? Number(e.target.value) : '')} size="small" select sx={{ minWidth: 200 }}>
                                    <MenuItem value="">-- No Customer --</MenuItem>
                                    {customers.map((c: any) => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}
                                </TextField>
                                <TextField label="Remarks (optional)" value={plRemarks} onChange={(e) => setPlRemarks(e.target.value)} size="small" sx={{ minWidth: 200 }} />
                            </Stack>
                            <Typography variant="body2" sx={{ fontWeight: 600, mb: 0.5, color: 'text.secondary' }}>VRP Items:</Typography>
                            {plItems.map((item, idx) => (
                                <Stack key={idx} direction={isMobile ? 'column' : 'row'} spacing={1} sx={{ mb: 0.5 }} alignItems={isMobile ? 'stretch' : 'center'}>
                                    <TextField
                                        label="VRP ID"
                                        value={item.vrp_id}
                                        onChange={(e) => handlePlItemChange(idx, 'vrp_id', e.target.value)}
                                        size="small"
                                        sx={{ flex: 1 }}
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => handlePlItemChange(idx, 'required_qty', parseInt(e.target.value, 10) || 1)}
                                        size="small"
                                        sx={{ width: 80 }}
                                        inputProps={{ min: 1 }}
                                    />
                                    {plItems.length > 1 && (
                                        <IconButton aria-label="Action" size="small" color="error" onClick={() => handleRemovePlRow(idx)}>
                                            <DeleteIcon fontSize="small" />
                                        </IconButton>
                                    )}
                                </Stack>
                            ...
                            <Stack direction="row" spacing={1} sx={{ mt: 1 }} flexWrap="wrap" useFlexGap>
                                <Button size="small" variant="outlined" startIcon={<AddIcon />} onClick={handleAddPlRow}>Add Row</Button>
                                <Button size="small" variant="outlined" color="secondary" startIcon={<PasteIcon />} onClick={() => setPasteDialogOpen(true)}>Paste VRP+Qty</Button>
                                <Button size="small" variant="outlined" color="secondary" startIcon={<UploadFileIcon />} onClick={() => fileInputRef.current?.click()}>Upload Excel</Button>
                                <input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" hidden onChange={handleExcelUpload} />
                                <Button variant="contained" color="primary" startIcon={<SendIcon />} onClick={handleCreatePicklist} disabled={plCreating}>
                            ...
                            <Dialog open={pasteDialogOpen} onClose={() => setPasteDialogOpen(false)} maxWidth="sm" fullWidth fullScreen={isMobile}>
                                ...
                                <DialogContent>
                                    <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                                        Paste tab/comma separated rows from Excel. Format: VRP ID [TAB] QTY (one per line).
                                    </Typography>
                                    <TextField
                                        multiline
                                        rows={10}
                                    ...
                                    {pasteText && (
                                        <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                                            Preview: {parsePastedItems(pasteText).length} valid rows detected
                                        </Typography>
                                    )}
                                </DialogContent>
                                <DialogActions>
                                    <Button onClick={() => { setPasteText(''); setPasteDialogOpen(false); }}>Cancel</Button>
                                    <Button variant="contained" onClick={handlePasteConfirm} disabled={!pasteText.trim()}>Add Rows</Button>
                                </DialogActions>
                            </Dialog>
                        </Paper>

                        {/* Picklists List */}
                        <Paper sx={{ flex: 1, p: 2, borderRadius: 2, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
                            <Stack direction={isMobile ? 'column' : 'row'} spacing={1} alignItems={isMobile ? 'stretch' : 'center'} sx={{ mb: 1 }}>
                                <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>Picklists</Typography>
                                <TextField value={plStatusFilter} onChange={(e) => setPlStatusFilter(e.target.value)} size="small" select sx={{ width: 130 }}>
                                ...
                                <Button size="small" variant="outlined" startIcon={<RefreshIcon />} onClick={loadPicklists} disabled={plLoading}>Refresh</Button>
                            </Stack>

                            <Box sx={{ flex: 1, overflow: 'auto' }}>
                                {plLoading && <LinearProgress sx={{ mb: 1 }} />}
                                {picklists.length === 0 && !plLoading && (
                                    <Typography variant="body2" sx={{ color: 'text.secondary', textAlign: 'center', py: 3 }}>No picklists found</Typography>
                                )}
                                {picklists.map((pl: any) => (
                                    ...
                                    >
                                        <Stack direction={isMobile ? 'column' : 'row'} justifyContent="space-between" alignItems={isMobile ? 'flex-start' : 'center'} spacing={0.75}>
                                            <Box>
                                                <Typography variant="body2" sx={{ fontWeight: 700 }}>{pl.picklist_id}</Typography>
                                                <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                                                    {pl.customer_name || 'No customer'} &bull; {pl.item_count} VRPs &bull; {pl.total_picked}/{pl.total_required} picked &bull; {formatDate(pl.created_at)}
                                                </Typography>
                                            </Box>
                                            <Stack direction="row" spacing={0.5} alignItems="center">
                                                <Chip label={pl.status} size="small" color={STATUS_COLORS[pl.status] || 'default'} variant="outlined" />
                                                {(pl.status === 'active' || pl.status === 'partial') && (
                                                    <IconButton aria-label="Action" size="small" color="error" onClick={(e) => { e.stopPropagation(); requestCancelPicklist(pl.id); }} title="Cancel">
                                        ...
                                        {pl.total_required > 0 && (
                                            <LinearProgress
                                                variant="determinate"
                                                value={Math.min(100, (pl.total_picked / pl.total_required) * 100)}
                                                sx={{ mt: 0.5, height: 4, borderRadius: 2 }}
                                                color={pl.total_picked >= pl.total_required ? 'success' : 'primary'}
                                            />
                                        )}
                                        ...
                                        {expandedPl?.id === pl.id && (
                                            <Box sx={{ mt: 1.5, pt: 1, borderTop: '1px solid', borderColor: 'divider' }}>
                                                {expandedLoading ? <CircularProgress size={20} /> : (
                                                    <>
                                                        {expandedPlItems.map((item: any) => (
                                                            <Stack key={item.id} direction={isMobile ? 'column' : 'row'} spacing={1} alignItems={isMobile ? 'flex-start' : 'center'} sx={{ mb: 0.5 }}>
                                                                ...
                                                                <Typography variant="body2" sx={{ fontWeight: 600, width: 120 }}>{item.vrp_id}</Typography>
                                                                <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                                                                    {item.picked_qty}/{item.required_qty} picked
                                                                </Typography>
                                                                <Chip label={item.status} size="small" color={STATUS_COLORS[item.status] || 'default'} variant="outlined" sx={{ ml: 'auto' }} />
                                                            </Stack>
                                                        ))}
                                                    </>
                                                )}
                ...
                {currentTabCode === 'picking' && (
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', p: { xs: 1, sm: 1.5 } }}>
                        {/* KPI Cards */}
                        <Stack direction={isMobile ? 'column' : 'row'} spacing={1.5} sx={{ mb: 1.5 }}>
                            <Card sx={{ flex: 1, minWidth: 110, background: 'linear-gradient(135deg, #059669, #10b981)', color: 'white' }}>
                                <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                    <Typography variant="caption" sx={{ opacity: 0.8 }}>Boxes Picked</Typography>
                                    <Typography variant="h5" sx={{ fontWeight: 700 }}>{pickTotals.total_boxes}</Typography>
                                </CardContent>
                            </Card>
                            <Card sx={{ flex: 1, minWidth: 110, background: 'linear-gradient(135deg, #7c3aed, #a78bfa)', color: 'white' }}>
                                <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                    <Typography variant="caption" sx={{ opacity: 0.8 }}>Total Qty</Typography>
                                    <Typography variant="h5" sx={{ fontWeight: 700 }}>{pickTotals.total_qty}</Typography>
                                </CardContent>
                            </Card>
                        </Stack>
                        ...
                        {!pickSessionId ? (
                            ...
                            <Box sx={{ textAlign: 'center', py: 4 }}>
                                <Typography variant="h6" sx={{ mb: 2, color: 'text.secondary' }}>No active picking session</Typography>
                                <Paper sx={{ p: 3, maxWidth: 500, mx: 'auto', borderRadius: 2 }}>
                                    <Stack spacing={2}>
                                        <TextField label="Select Picklist" value={pickPicklistId} onChange={(e) => setPickPicklistId(e.target.value)} size="small" select fullWidth>
                                            <MenuItem value="">-- Select Active Picklist --</MenuItem>
                                            {activePicklists.map((pl: any) => (
                                                <MenuItem key={pl.id} value={pl.id}>
                                                    {pl.picklist_id} — {pl.customer_name || 'No customer'} ({pl.total_picked}/{pl.total_required})
                                                </MenuItem>
                                            ))}
                                        </TextField>
                                        <Button variant="contained" size="large" color="success" startIcon={<InventoryIcon />} onClick={handleStartPicking} disabled={pickLoading || !pickPicklistId}>
                                            Start Picking
                                        </Button>
                                    </Stack>
                                </Paper>
                            ...
                            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                                ...
                                {pickGuideSummary.length > 0 && (
                                    <Paper sx={{ p: 1.5, mb: 1, borderRadius: 2, maxHeight: 180, overflow: 'auto' }}>
                                        <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 0.5, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                            <InventoryIcon fontSize="small" color="primary" /> Pick Guide
                                        </Typography>
                                        {pickGuideSummary.map((item: any) => (
                                            <Box key={item.id} sx={{ mb: 0.5 }}>
                                                <Stack direction={isMobile ? 'column' : 'row'} spacing={1} alignItems={isMobile ? 'flex-start' : 'center'}>
                                                    ...
                                                    <Typography variant="body2" sx={{ fontWeight: 700, minWidth: 100 }}>{item.vrp_id}</Typography>
                                                    <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                                                        Need: {item.remaining > 0 ? item.remaining : 0} &bull; Available: {item.available_count}
                                                    </Typography>
                                                    <Typography variant="body2" sx={{ color: 'primary.main', fontWeight: 600 }}>
                                                        {Object.entries(item.racks).map(([rack, data]: any) => `${rack}(${data.count})`).join(', ')}
                                                    </Typography>
                                                </Stack>
                                            </Box>
                                        ))}
                                ...
                                <Paper sx={{ p: 1.5, mt: 1, display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 1 : 0, alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', borderRadius: 2, bgcolor: isDarkMode ? '#1e293b' : '#f1f5f9' }}>
                                    <Stack direction="row" spacing={1} alignItems="center">
                                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                                            {pickEntries.length} entries — {pickTotals.total_boxes} boxes — {pickTotals.total_qty} qty
                                        </Typography>
                                        <Button variant="outlined" color="error" size="small" startIcon={<CancelIcon />} onClick={requestDiscardPicking}>
                                            Discard
                                        </Button>
                                    </Stack>
                                    <Button variant="contained" color="success" startIcon={<SendIcon />} onClick={() => setPickSubmitDialogOpen(true)} disabled={pickEntries.length === 0 || pickSubmitting}>
                                        Submit Picking
                                    </Button>
                                </Paper>
                            </Box>
                ...
                {currentTabCode === 'picking_list' && (
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', p: { xs: 1, sm: 1.5 } }}>
                        <Button variant="outlined" size="small" startIcon={<RefreshIcon />} onClick={loadPickingList} disabled={pickingListLoading} sx={{ mb: 1, alignSelf: 'flex-start' }}>Refresh</Button>
                        <Box sx={{ flex: 1, minHeight: 0 }}>
                            <Box className="ag-theme-quartz" sx={gridSx}>
                                <AgGridReact
            ...
            {/* Submit Dialog */}
            <Dialog open={pickSubmitDialogOpen} onClose={() => setPickSubmitDialogOpen(false)} maxWidth="xs" fullWidth>
                <DialogTitle>Submit Picking?</DialogTitle>
                <DialogContent>
                    <Typography>This will mark <strong>{pickTotals.total_boxes} boxes</strong> ({pickTotals.total_qty} qty) as picked and move them to staging area.</Typography>
                    <Typography variant="body2" sx={{ mt: 1, color: 'text.secondary' }}>Box status will change to &apos;picked&apos;. Picklist progress will be updated.</Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setPickSubmitDialogOpen(false)}>Cancel</Button>
                    <Button variant="contained" color="success" onClick={handleSubmitPicking} disabled={pickSubmitting} startIcon={pickSubmitting ? <CircularProgress size={16} /> : <SendIcon />}>
                        {pickSubmitting ? 'Submitting...' : 'Confirm'}
                    </Button>
                </DialogActions>
            </Dialog>

            <Dialog open={!!cancelPicklistTargetId} onClose={() => setCancelPicklistTargetId(null)} maxWidth="xs" fullWidth>
                <DialogTitle>Cancel Picklist?</DialogTitle>
                <DialogContent>
                    <Typography>This will cancel the selected picklist and stop further picking against it.</Typography>
                </DialogContent>
                <DialogActions>
            ...
            <Dialog open={discardPickingDialogOpen} onClose={() => setDiscardPickingDialogOpen(false)} maxWidth="xs" fullWidth>
                <DialogTitle>Discard Picking Session?</DialogTitle>
                <DialogContent>
                    <Typography>This will remove all scanned picking entries from the current draft session.</Typography>
                    <Typography variant="body2" sx={{ mt: 1, color: 'text.secondary' }}>This action cannot be undone.</Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setDiscardPickingDialogOpen(false)}>Cancel</Button>
                    <Button
                        variant="contained"
                        color="error"
                        onClick={async () => {
