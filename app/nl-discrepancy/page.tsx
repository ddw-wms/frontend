...
import {
    ...
    CircularProgress, Alert, Card, CardContent, useTheme, useMediaQuery,
...
const STATUS_COLORS: Record<string, 'error' | 'success' | 'warning' | 'info' | 'default'> = {
    open: 'error',
    resolved: 'success',
    escalated: 'warning',
    investigating: 'info',
...
export default function NLDiscrepancyPage() {
    ...
    const { filterTabs, canSeeTab, isAdmin } = usePagePermissions('nl_discrepancy');
    ...
    const [stats, setStats] = useState<any>({
        total: 0, open_count: 0, resolved_count: 0, escalated_count: 0,
    });
    ...
    const loadStats = useCallback(async () => {
        if (!activeWarehouse?.id) return;
        try {
            const res = await nlDiscrepancyAPI.getStats({ warehouse_id: activeWarehouse.id });
    ...
    const columnDefs = useMemo(() => [
        ...
        {
            field: 'discrepancy_type', headerName: 'Type', width: 120,
            cellRenderer: (p: any) => <Chip label={(p.value || '').replace(/_/g, ' ')} size="small" variant="outlined" />,
        },
        ...
        { field: 'notes', headerName: 'Notes', flex: 1, minWidth: 140 },
        {
            ...
            cellRenderer: (p: any) => (
                <Chip label={p.value} size="small" color={STATUS_COLORS[p.value] || 'default'} variant="outlined" />
            ),
        },
        { field: 'raised_by_name', headerName: 'Raised By', width: 120 },
        { field: 'created_at', headerName: 'Date', width: 140, valueFormatter: (p: any) => formatDate(p.value) },
                ...
                return (
                    <Stack direction="row" spacing={0.5} sx={{ mt: 0.3 }}>
                        <Button size="small" variant="outlined" color="success"
                            onClick={() => { setResolveId(p.data.id); setResolveDialogOpen(true); }}>
                            Resolve
                        </Button>
    ...
    return (
        <AppLayout>
            ...
            <Box sx={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: isDarkMode ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' : 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)' }}>
                <StandardPageHeader title="NL Discrepancy" subtitle="Track & resolve inventory discrepancies" icon="⚠️" warehouseName={activeWarehouse?.name} />
                <StandardTabs value={tabValue} onChange={(_, v) => setTabValue(v)} tabs={visibleTabs} color="#ef4444" />
                ...
                {currentTabCode === 'list' && (
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', p: { xs: 1, sm: 1.5 } }}>
                        ...
                        <Stack direction="row" spacing={1.5} sx={{ mb: 1.5, flexWrap: 'wrap' }}>
                            <Card sx={{ flex: 1, minWidth: 100, background: 'linear-gradient(135deg, #1e40af, #3b82f6)', color: 'white' }}>
                            ...
                            <Card sx={{ flex: 1, minWidth: 100, background: 'linear-gradient(135deg, #dc2626, #f87171)', color: 'white' }}>
                                <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                    <Typography variant="caption" sx={{ opacity: 0.8 }}>Open</Typography>
                                    <Typography variant="h5" sx={{ fontWeight: 700 }}>{stats.open_count}</Typography>
                                </CardContent>
                            </Card>
                            <Card sx={{ flex: 1, minWidth: 100, background: 'linear-gradient(135deg, #059669, #10b981)', color: 'white' }}>
                                <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                    <Typography variant="caption" sx={{ opacity: 0.8 }}>Resolved</Typography>
                                    <Typography variant="h5" sx={{ fontWeight: 700 }}>{stats.resolved_count}</Typography>
                                </CardContent>
                            </Card>
                            <Card sx={{ flex: 1, minWidth: 100, background: 'linear-gradient(135deg, #d97706, #f59e0b)', color: 'white' }}>
                                <CardContent sx={{ py: 1, px: 2, '&:last-child': { pb: 1 } }}>
                                    <Typography variant="caption" sx={{ opacity: 0.8 }}>Escalated</Typography>
                                    <Typography variant="h5" sx={{ fontWeight: 700 }}>{stats.escalated_count}</Typography>
                                </CardContent>
                            </Card>
                        </Stack>
                        ...
                        <Paper sx={{ p: 1.5, mb: 1, borderRadius: 2 }}>
                            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ sm: 'center' }}>
                                <TextField
                                    ...
                                    onKeyDown={(e) => { if (e.key === 'Enter') loadDiscrepancies(); }}
                                />
                                <TextField label="Status" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} size="small" select sx={{ minWidth: 120 }}>
                                    <MenuItem value="">All</MenuItem>
                                    <MenuItem value="open">Open</MenuItem>
                                    <MenuItem value="resolved">Resolved</MenuItem>
                                    <MenuItem value="escalated">Escalated</MenuItem>
                                </TextField>
                                <TextField label="Type" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} size="small" select sx={{ minWidth: 130 }}>
                                    <MenuItem value="">All</MenuItem>
                                    {DISCREPANCY_TYPES.map((t) => (
                                        <MenuItem key={t.value} value={t.value}>{t.label}</MenuItem>
                                    ))}
                                </TextField>
                                <Button variant="outlined" size="small" startIcon={<RefreshIcon />} onClick={loadDiscrepancies} disabled={listLoading}>Refresh</Button>
                            </Stack>
                        </Paper>
                        ...
                        <Paper sx={{ p: 1, mt: 1, borderRadius: 2, bgcolor: isDarkMode ? '#1e293b' : '#f1f5f9' }}>
                ...
                {currentTabCode === 'create' && (
                    <Box sx={{ flex: 1, overflow: 'auto', p: { xs: 1, sm: 1.5 } }}>
                        <Paper sx={{ p: 3, maxWidth: 600, mx: 'auto', borderRadius: 2 }}>
                            ...
                            <Stack spacing={2}>
                                <TextField label="Discrepancy Type *" value={formType} onChange={(e) => setFormType(e.target.value)} size="small" select fullWidth>
                                    {DISCREPANCY_TYPES.map((t) => (
                                        <MenuItem key={t.value} value={t.value}>{t.label}</MenuItem>
                                    ))}
                                </TextField>
                                <Stack direction="row" spacing={2}>
                                    <TextField label="Box ID" value={formBoxId} onChange={(e) => setFormBoxId(e.target.value)} size="small" fullWidth />
                                    <TextField label="VRP ID" value={formVrpId} onChange={(e) => setFormVrpId(e.target.value)} size="small" fullWidth />
                                </Stack>
                                <Stack direction="row" spacing={2}>
                                    <TextField label="Expected Value" value={formExpected} onChange={(e) => setFormExpected(e.target.value)} size="small" fullWidth />
                                    <TextField label="Actual Value" value={formActual} onChange={(e) => setFormActual(e.target.value)} size="small" fullWidth />
                                </Stack>
                                <TextField label="Notes" value={formNotes} onChange={(e) => setFormNotes(e.target.value)} size="small" fullWidth multiline rows={3} />
                                <Button variant="contained" color="error" size="large" startIcon={creating ? <CircularProgress size={16} color="inherit" /> : <AddIcon />} onClick={handleCreate} disabled={creating || !formType} fullWidth>
            ...
            {/* Resolve Dialog */}
            <Dialog open={resolveDialogOpen} onClose={() => setResolveDialogOpen(false)} maxWidth="sm" fullWidth fullScreen={isMobile}>
                <DialogTitle>Resolve Discrepancy</DialogTitle>
                <DialogContent>
                    <Stack spacing={2} sx={{ mt: 1 }}>
                        <TextField label="Resolution *" value={resolveResolution} onChange={(e) => setResolveResolution(e.target.value)} size="small" fullWidth multiline rows={2} placeholder="Describe how this was resolved..." />
                        <TextField label="Additional Notes" value={resolveNotes} onChange={(e) => setResolveNotes(e.target.value)} size="small" fullWidth multiline rows={2} />
                    </Stack>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setResolveDialogOpen(false)}>Cancel</Button>
                    <Button variant="contained" color="success" onClick={handleResolve} disabled={resolving || !resolveResolution.trim()} startIcon={resolving ? <CircularProgress size={16} /> : <CheckCircleIcon />}>
                        {resolving ? 'Resolving...' : 'Confirm Resolve'}
                    </Button>
                </DialogActions>
            </Dialog>

            <Dialog open={!!escalateTargetId} onClose={() => setEscalateTargetId(null)} maxWidth="xs" fullWidth>
                <DialogTitle>Escalate Discrepancy?</DialogTitle>
                <DialogContent>
                    <Typography>This will mark the discrepancy as escalated for further review.</Typography>
                </DialogContent>
                <DialogActions>
