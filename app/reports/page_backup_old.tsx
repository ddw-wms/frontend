...
export default function ReportsPage() {
    ...
    const getColumnDefs = useMemo(() => {
        ...
        switch (selectedReport) {
            case 'current_stock':
                return [
                    { field: 'wsn', headerName: 'WSN', width: 140, pinned: 'left' as 'left', ...commonColConfig },
            ...
            case 'performance':
                return [
                    { field: 'user_name', headerName: 'User Name', width: 180, pinned: 'left' as 'left', ...commonColConfig },
                    {
                        field: 'activity_type',
            ...
            return (
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2 }}>
                    {summaryData.map((item: any, index: number) => (
                        <Box sx={{ flex: '1 1 200px', minWidth: 150 }} key={index}>
                            <Card>
                                <CardContent>
    ...
    return (
        <AppLayout>
            <Box sx={{ height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column' }}>
                ...
                {/* Tabs */}
                <StandardTabs
                    value={Object.keys(reportConfigs).indexOf(selectedReport)}
                    onChange={(event, newValue) => setSelectedReport(Object.keys(reportConfigs)[newValue])}
                    tabs={Object.values(reportConfigs).map(config => config.name)}
                    color="#667eea"
                />
